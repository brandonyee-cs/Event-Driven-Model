...
class EventDataLoader:
    def __init__(
        ...
        if isinstance(stock_paths, str):
            ...
            raise TypeError(
                "stock_paths must be a string or a list of Parquet file paths."
            )
    ...
    def _load_single_stock_parquet(self, stock_path: str) -> Optional[pl.DataFrame]:
        ...
        try:
            ...
            essential_cols_found = all(
                found_std_names.get(c, False) for c in ["date", "ticker", "prc", "ret"]
            )
            ...
            if missing_cols:
                essential_pr = [
                    "prc",
                    "ret",
                ]  # Re-check just in case logic missed something
                if any(col in missing_cols for col in essential_pr):
                    raise ValueError(
                        f"Essential columns {missing_cols} missing in {stock_path} AFTER selection/rename."
                    )
                else:
    ...
    def load_data(self) -> Optional[pl.DataFrame]:
        ...
        try:
            ...
            event_df_peek = pl.read_csv_batched(
                self.event_path, batch_size=1
            ).next_batches(1)[0]
            ...
            if ticker_col not in event_df_peek.columns:
                ...
                potential_ticker_cols = [
                    c
                    for c in event_df_peek.columns
                ...
                if potential_ticker_cols:
                    ...
                    raise ValueError(
                        f"Ticker column '{self.ticker_col}' not found in event file."
                    )
            ...
            if date_col not in event_df_peek.columns:
                ...
                if potential_date_cols:
                    ...
                    raise ValueError(
                        f"Event date column '{self.event_date_col}' not found."
                    )

            # print(f"Using columns '{ticker_col}' (as ticker) and '{date_col}' (as Event Date) from event file.")
            event_data_raw = pl.read_csv(
                self.event_path, columns=[ticker_col, date_col], try_parse_dates=True
            )
            event_data_renamed = event_data_raw.rename(
                {ticker_col: "ticker", date_col: "Event Date"}
            )

            # --- Check/Correct Date Type ---
            if event_data_renamed["Event Date"].dtype == pl.Object or isinstance(
                event_data_renamed["Event Date"].dtype, pl.String
            ):
                ...
                event_data_processed = event_data_renamed.with_columns(
                    [
                        ...
                        .cast(pl.Datetime),  # Explicit parse and cast
                        pl.col("ticker").cast(pl.Utf8).str.to_uppercase(),
                    ]
                )
            elif isinstance(
                event_data_renamed["Event Date"].dtype, (pl.Date, pl.Datetime)
            ):
                # print("    'Event Date' already parsed as Date/Datetime.")
                event_data_processed = event_data_renamed.with_columns(
                    [
                        pl.col("Event Date").cast(
                            pl.Datetime
                        ),  # Ensure it's Datetime specifically if needed
                        pl.col("ticker").cast(pl.Utf8).str.to_uppercase(),
                    ]
                )
            else:
                raise TypeError(
                    f"Unexpected dtype for 'Event Date': {event_data_renamed['Event Date'].dtype}"
                )
            # --- End Corrected Date Handling ---

            event_data_processed = event_data_processed.drop_nulls(
                subset=["Event Date"]
            )
            events = event_data_processed.unique(
                subset=["ticker", "Event Date"], keep="first"
            )
            ...
            raise FileNotFoundError(f"Event file not found: {self.event_path}")
        except Exception as e:
            raise ValueError(f"Error processing event file {self.event_path}: {e}")

        # --- Process Events in Chunks ---
        processed_chunks = []
        num_chunks = (
            n_total_events + self.event_chunk_size - 1
        ...
        for i in range(num_chunks):
            ...
            for stock_path in self.stock_paths:
                try:
                    ...
                    for std_name, variations in standard_names.items():
                        for var in variations:
                            if var in col_map_lower:
                                ...
                                break
                    if not any(
                        sc in ["date", "trade_date", "trading_date", "tradedate", "dt"]
                        for sc in col_map_lower
                    ) or not any(
                        sc in ["ticker", "symbol", "sym_root", "tic", "permno"]
                        for sc in col_map_lower
                    ):
                    ...
                    scan_expressions = []
                    if date_dtype == pl.Object or isinstance(
                        date_dtype, pl.String
                    ):  # String check for Polars
                        scan_expressions.append(
                            pl.col("date")
                            .str.to_datetime(strict=False)
                            .cast(pl.Datetime)
                        )
                    elif (
                        date_dtype != pl.Datetime
                    ):  # if not already datetime (e.g. Date)
                    ...
                    filtered_scan = scan.filter(
                        ...
                        & pl.col("date").is_between(
                            required_min_date, required_max_date, closed="both"
                        )
                    )
            ...
            stock_data_chunk = stock_data_chunk.with_columns(
                [pl.col("ticker").cast(pl.Utf8), pl.col("date").cast(pl.Datetime)]
            )
            stock_data_chunk = stock_data_chunk.unique(
                subset=["date", "ticker"], keep="first", maintain_order=False
            )
            ...
            event_chunk = event_chunk.with_columns(
                [pl.col("ticker").cast(pl.Utf8), pl.col("Event Date").cast(pl.Datetime)]
            )
            merged_chunk = stock_data_chunk.join(event_chunk, on="ticker", how="inner")
            ...
            processed_chunk = processed_chunk.with_columns(
                [
                    ...
                    (
                        pl.col("ticker")
                        + "_"
                        + pl.col("Event Date").dt.strftime("%Y%m%d")
                    ).alias("event_id"),
                ]
            )
            ...
            event_cols_present = (
                event_chunk.columns
            )  # These were 'ticker' and 'Event Date'
            ...
            final_cols_set = (
                set(stock_cols_present)
                | set(event_cols_present)
                | set(derived_cols_created)
            )
            final_cols_to_select = [
                c for c in final_cols_set if c in processed_chunk.columns
            ]
        ...
        combined_data = pl.concat(processed_chunks, how="vertical").sort(
            ["ticker", "Event Date", "date"]
        )
...
class EventFeatureEngineer:
    def __init__(self, prediction_window: int = 3):
        ...
        self.categorical_features: List[str] = (
            []
        )  # Store names of created categorical/dummy features
    ...
    def calculate_features(
        ...
        if missing:
            raise ValueError(
                f"Missing required columns for feature calculation: {missing}"
            )
        ...
        feature_expressions.append(
            (pl.col("momentum_10") - pl.col("momentum_20")).alias(
                "delta_momentum_10_20"
            )
        )
        ...
        for window in self.windows:
            ...
            feature_expressions.append(
                pl.col(return_col)
                .rolling_std(window_size=window, min_samples=min_p)
                .over("event_id")
                .alias(col_name)
            )
            current_features.append(col_name)
        ...
        feature_expressions.append(
            (pl.col("volatility_5") - pl.col("volatility_10")).alias(
                "delta_volatility_5_10"
            )
        )
        current_features.append("delta_volatility_5_10")
        feature_expressions.append(
            (pl.col("volatility_10") - pl.col("volatility_20")).alias(
                "delta_volatility_10_20"
            )
        )
        ...
        for lag in range(1, 4):
            ...
            feature_expressions.append(
                pl.col(return_col).shift(lag).over("event_id").alias(col_name)
            )
            current_features.append(col_name)
        ...
        if has_volume:
            ...
            df = df.with_columns(
                [
                    ...
                    for c in ["norm_vol", "vol_momentum_5", "vol_momentum_10"]
                ]
            )
        ...
        df = df.with_columns(
            [
                ...
                if df.select(pl.col(c)).dtypes[0] in [pl.Float32, pl.Float64]
            ]
        )
        ...
        final_cols_to_keep = sorted(
            list(set(c for c in keep_cols if c in df.columns))
        )  # Unique and existing

        return df.select(final_cols_to_keep)

    def get_features_target(
        self, df: pl.DataFrame, fit_imputer: bool = False
        ...
        if not self.feature_names:
            raise RuntimeError("Run calculate_features first.")

        available_features = [f for f in self.feature_names if f in df.columns]
        if not available_features:
            raise ValueError("No calculated features found in DataFrame.")
        ...
        if df_with_target.is_empty():
            ...
            numeric_features = [
                f for f in available_features if f not in self.categorical_features
            ]
            categorical_cols_present = [
                f for f in available_features if f in self.categorical_features
            ]
            num_cols = len(numeric_features) + len(categorical_cols_present)
            return (
                np.array([]).reshape(0, num_cols),
                np.array([]),
                numeric_features + categorical_cols_present,
            )
        ...
        if fit_imputer:
            ...
            if X_numeric_np.size > 0:
                ...
                self._imputer_fitted = (
                    True  # Mark as fitted even if empty to prevent error
                )
        else:
            if not self._imputer_fitted:
                raise RuntimeError(
                    "Imputer not fitted. Call with fit_imputer=True first."
                )
        ...
        if categorical_df.width > 0:
            try:
                X_categorical_np = categorical_df.select(
                    [
                        pl.col(c).cast(pl.UInt8, strict=False)
                        for c in categorical_cols_present
                    ]
                ).to_numpy()
            except Exception as e:
                raise ValueError(
                    f"Failed to convert categorical features to NumPy: {e}"
                )
        else:
            X_categorical_np = np.empty(
                (X_numeric_imputed_np.shape[0], 0), dtype=np.uint8
            )
...
class EventAnalysis:
    def __init__(
        self, data_loader: EventDataLoader, feature_engineer: EventFeatureEngineer
    ):
    ...
    def load_and_prepare_data(
        self, run_feature_engineering: bool = False
    ...
    def train_models(
        self, test_size: float = 0.2, time_split_column: str = "Event Date"
    ):
        ...
        try:
            X, y, feature_names = self.feature_engineer.get_features_target(
                self.data, fit_imputer=True
            )
            ...
            train_indices_in_X = (
                None  # Initialize to handle case where time_split_column might not work
            )
            if time_split_column in self.data.columns:
                ...
                data_for_split_base = self.data.filter(
                    pl.col("future_ret").is_not_null()
                )
                dates_for_split = (
                    data_for_split_base.get_column(time_split_column)
                    .to_numpy()
                    .flatten()
                )
                ...
                if len(dates_for_split) != X.shape[0]:
                    ...
                    X_train_pl = data_for_split_base[train_indices_in_X].select(
                        feature_names
                    )
                    y_train_pl = data_for_split_base[train_indices_in_X].get_column(
                        "future_ret"
                    )
            ...
            tsridge_model = TimeSeriesRidge(
                alpha=0.1, lambda2=0.5, feature_order=feature_names
            )
            ...
            xgb_model_instance = XGBoostDecileModel(
                weight=0.7,
                xgb_params=xgb_params_config,
                ts_ridge_feature_order=feature_names,
            )
    ...
    def plot_feature_importance(
        self, results_dir: str, file_prefix: str, model_name: str
    ):
        ...
        try:
            ...
            plot_filename = os.path.join(
                results_dir, f"{file_prefix}_feat_importance_{model_name}.png"
            )
            try:
    ...
    def plot_predictions_for_event(
        self, results_dir: str, event_id: str, file_prefix: str, model_name: str
    ):
        ...
        if (
            ...
            return
        try:
            event_data_pl = self.data.filter(pl.col("event_id") == event_id).sort(
                "days_to_event"
            )
            if (
                event_data_pl.is_empty()
            ...
            event_date_str = (
                event_date_val.strftime("%Y-%m-%d")
                if isinstance(event_date_val, datetime.datetime)
                else str(event_date_val)
            )
            ax.set_title(
                f"Return Predictions - {ticker} Event on {event_date_str} - {model_name}"
            )
            ...
            plt.tight_layout()
            plot_filename = os.path.join(
                results_dir, f"{file_prefix}_pred_{model_name}_{event_id}.png"
            )
            try:
                plt.savefig(plot_filename, dpi=200)
                ...
                Exception
            ) as e:  # print(f"Warning: Could not save plot image {plot_filename}: {e}");
                pass
            plt.close(fig)
        except (
            Exception
        ) as e:  # print(f"Error plotting predictions: {e}"); traceback.print_exc();
            pass

    def calculate_rolling_sharpe_timeseries(
        self,
        results_dir: str,
        ...
        if (
            self.data is None
            or return_col not in self.data.columns
            or "days_to_event" not in self.data.columns
        ):
        ...
        sample_returns = analysis_data.select(pl.col(return_col)).sample(
        ...
        analysis_data = analysis_data.with_columns(
            pl.col(return_col_for_calc)
            .clip_quantile(0.01, 0.99)
            .alias("clipped_return")
        )
        ...
        for day in range(analysis_window[0], analysis_window[1] + 1):
            ...
            if window_data_for_day.is_empty():
                sharpe_data.append(
                    {"days_to_event": day, "sharpe_ratio": None, "event_count": 0}
                )
                continue
            event_sharpe = (
                window_data_for_day.group_by("event_id")
                .agg(
                    [
        ...
        sharpe_df = all_days.join(sharpe_df, on="days_to_event", how="left").sort(
            "days_to_event"
        )
        ...
        try:
            ...
            fig, ax = plt.subplots(figsize=(12, 7))
            ax.plot(
                results_pd["days_to_event"],
                results_pd["sharpe_ratio"],
                color="blue",
                linewidth=1,
                alpha=0.3,
                label="Raw Sharpe Ratio",
            )
            ax.plot(
                results_pd["days_to_event"],
                results_pd["smooth_sharpe"],
                color="red",
                linewidth=2,
            ...
            y_min_plot, y_max_plot = (
                results_pd["smooth_sharpe"].dropna().min(),
                results_pd["smooth_sharpe"].dropna().max(),
            )
            if pd.isna(y_min_plot) or pd.isna(y_max_plot) or y_min_plot == y_max_plot:
                y_lim_final = [-1, 1]
            else:
                y_padding = 0.1 * (y_max_plot - y_min_plot)
            ...
            ax.set_title(
                f"Rolling Sharpe Ratio vs. Days to Event ({sharpe_window}-Day Window)"
            )
            ax.set_xlabel("Days Relative to Event")
            ...
            ax.grid(True, linestyle=":", alpha=0.7)
            plot_filename = os.path.join(
                results_dir, f"{file_prefix}_rolling_sharpe_timeseries.png"
            )
            ...
            sharpe_df.write_csv(
                os.path.join(
                    results_dir, f"{file_prefix}_rolling_sharpe_timeseries.csv"
                )
            )
            # print(f"Saved rolling Sharpe data to: {os.path.join(results_dir, f'{file_prefix}_rolling_sharpe_timeseries.csv')}")
        except (
            Exception
        ) as e:  # print(f"Error plotting/saving Sharpe: {e}"); traceback.print_exc();
            pass
        return sharpe_df

    def calculate_sharpe_quantiles(
        self,
        results_dir: str,
        file_prefix: str = "event",
        return_col: str = "ret",
        ...
        if (
            self.data is None or return_col not in self.data.columns
        ):  # print("Error: Data/return_col missing.");
            return None
        daily_rf = (
            (1 + risk_free_rate) ** (1 / 252) - 1
            if annualize and risk_free_rate > 0
        ...
        sample_returns = temp_analysis_data.select(pl.col(return_col)).sample(
        ...
        if returns_in_pct:  # print("Converting returns to decimal for Sharpe calc");
            analysis_data = self.data.with_columns(
                (pl.col(return_col) / 100).alias("decimal_return")
            )
            return_col_for_calc = "decimal_return"
        else:
        ...
        if (
            analysis_data.is_empty()
        ):  # print(f"Error: No data in window {analysis_window}.");
            return None
        ...
        for day in days_range:
            ...
            if window_data_for_day.is_empty():
                ...
                [empty_res.update({f"sharpe_q{int(q*100)}": None}) for q in quantiles]
                sharpe_data.append(empty_res)
                continue
            sharpe_by_event_df = (
                window_data_for_day.group_by("event_id")
                .agg(
            ...
            if valid_events_count >= 5:
                sharpe_by_event_df = sharpe_by_event_df.with_columns(
                    (
                        (pl.col("mean_ret") - daily_rf)
                        / pl.col("std_dev")
                        * (np.sqrt(252) if annualize else 1)
                    ).alias("sharpe")
                ).with_columns(
                    pl.col("sharpe").clip_quantile(0.05, 0.95).alias("sharpe")
                )  # Clip extreme Sharpe per event
                q_values_dict = {
                    f"sharpe_q{int(q*100)}": sharpe_by_event_df.select(
                        pl.col("sharpe").quantile(q, interpolation="linear")
                    ).item()
                    for q in quantiles
                }
                day_res = {
                ...
                [day_res.update({f"sharpe_q{int(q*100)}": None}) for q in quantiles]
            sharpe_data.append(day_res)
            del window_data_for_day, sharpe_by_event_df
            gc.collect()
        ...
        try:
            ...
            ax.axvline(x=0, color="red", linestyle="--", label="Event Day")
            # Auto-adjust y-limits or set reasonable defaults
            all_q_data = pd.concat(
                [
                    results_pd[f"sharpe_q{int(q*100)}"]
                    for q in quantiles
                    if f"sharpe_q{int(q*100)}" in results_pd.columns
                ]
            ).dropna()
            y_min_plot, y_max_plot = all_q_data.min() if not all_q_data.empty else -1, (
                all_q_data.max() if not all_q_data.empty else 1
            )
            ...
            ax.set_title(
                f"Sharpe Ratio Quantiles Around Events (Lookback: {lookback_window} days)"
            )
            ax.set_xlabel("Days Relative to Event")
            ax.set_ylabel("Sharpe Ratio" + (" (Annualized)" if annualize else ""))
            ax.legend(loc="best")
            ax.grid(True, linestyle=":", alpha=0.7)
            plot_filename = os.path.join(
                results_dir, f"{file_prefix}_sharpe_quantiles.png"
            )
            plt.savefig(plot_filename, dpi=200, bbox_inches="tight")
            plt.close(fig)
            # print(f"Saved Sharpe quantiles plot to: {plot_filename}")
            results_df.write_csv(
                os.path.join(results_dir, f"{file_prefix}_sharpe_quantiles.csv")
            )
            # print(f"Saved Sharpe quantiles data to: {os.path.join(results_dir, f'{file_prefix}_sharpe_quantiles.csv')}")
        except (
            Exception
        ) as e:  # print(f"Error plotting/saving Sharpe quantiles: {e}"); traceback.print_exc();
            pass
        return results_df

    def analyze_volatility_spikes(
        self,
        results_dir: str,
        file_prefix: str = "event",
        window: int = 5,
        ...
        if (
            self.data is None
            or "ret" not in self.data.columns
            or "days_to_event" not in self.data.columns
        ...
        vol_change_pct_val = (
            ((event_vol_val / baseline_vol_val) - 1) * 100
            if baseline_vol_val is not None
            and baseline_vol_val > 0
            and event_vol_val is not None
            else float("nan")
        )
        ...
        if not aligned_vol.is_empty():
            try:
                ...
                fig, ax = plt.subplots(figsize=(12, 7))
                ax.plot(
                    aligned_vol_pd["days_to_event"],
                    aligned_vol_pd["avg_annualized_vol"],
                    color="blue",
                    linewidth=2,
                ...
                if baseline_vol_val is not None:
                    ax.axhline(
                        ...
                        color="green",
                        linestyle="--",
                        label=f"Baseline Vol: {baseline_vol_val:.1f}%",
                    )
                ax.axvline(x=0, color="red", linestyle="--", label="Event Day")
                ax.axvspan(
                    event_window[0],
                ...
                ax.set_title(
                    f"Average Rolling Volatility Around Event (Window={window} days)"
                )
                ax.set_xlabel("Days Relative to Event")
                ...
                plot_filename = os.path.join(
                    results_dir, f"{file_prefix}_volatility_rolling_{window}d.png"
                )
                plt.savefig(plot_filename, dpi=200, bbox_inches="tight")
                plt.close(fig)
                # print(f"Saved rolling volatility plot to: {plot_filename}")
                aligned_vol.write_csv(
                    os.path.join(
                        results_dir,
                        f"{file_prefix}_volatility_rolling_{window}d_data.csv",
                    )
                )
                # print(f"Saved rolling volatility data to: {os.path.join(results_dir, f'{file_prefix}_volatility_rolling_{window}d_data.csv')}")
            except (
                Exception
            ) as e:  # print(f"Error plotting/saving VolatilitySpikes: {e}"); traceback.print_exc();
                pass
        # else: print("No data for rolling volatility plot.")
        return aligned_vol

    def calculate_volatility_quantiles(
        self,
        results_dir: str,
        file_prefix: str = "event",
        return_col: str = "ret",
        ...
        analysis_data_filtered = analysis_data_filtered.with_columns(
            pl.col(return_col).clip_quantile(0.01, 0.99).alias("clipped_return")
        )
        if (
            analysis_data_filtered.is_empty()
        ):  # print(f"Error: No data in window {analysis_window}.");
            return None
        ...
        for day_val in days_range_list:
            ...
            if current_window_data.is_empty():
                ...
                [
                    empty_res_dict.update({f"vol_q{int(q*100)}": None})
                    for q in [0.05, 0.5, 0.95]
                ]
                vol_quantile_data.append(empty_res_dict)
                continue
            vol_by_event_df = (
                current_window_data.group_by("event_id")
                .agg([pl.std("clipped_return").alias("vol"), pl.count().alias("n_obs")])
                .filter(
                    (pl.col("n_obs") >= max(3, lookback_window // 3))
            ...
            if num_valid_events >= 5:
                ...
                quantile_values_dict = {
                    f"vol_q{int(q*100)}": vol_by_event_df.select(
                        pl.col("annualized_vol").quantile(q, interpolation="linear")
                    ).item()
                    for q in [0.05, 0.5, 0.95]
                }
                day_results_dict = {
                    "days_to_event": day_val,
                    "event_count": num_valid_events,
                    **quantile_values_dict,
                }
            else:
                day_results_dict = {
                ...
                [
                    day_results_dict.update({f"vol_q{int(q*100)}": None})
                    for q in [0.05, 0.5, 0.95]
                ]
            vol_quantile_data.append(day_results_dict)
            del current_window_data, vol_by_event_df
            gc.collect()
        ...
        try:
            ...
            for q_item in [0.05, 0.5, 0.95]:
                ...
                if col_name_str in results_vol_pd.columns:
                    ax.plot(
                        results_vol_pd["days_to_event"],
                        results_vol_pd[col_name_str],
            ...
            ax.axvline(x=0, color="red", linestyle="--", label="Event Day")
            ax.set_title(
                f"Volatility Quantiles Around Events (Lookback: {lookback_window} days)"
            )
            ax.set_xlabel("Days Relative to Event")
            ...
            ax.set_ylim(bottom=0)
            plot_filename = os.path.join(
                results_dir, f"{file_prefix}_volatility_quantiles.png"
            )
            plt.savefig(plot_filename, dpi=200, bbox_inches="tight")
            plt.close(fig)
            # print(f"Saved volatility quantiles plot to: {plot_filename}")
            results_vol_df.write_csv(
                os.path.join(results_dir, f"{file_prefix}_volatility_quantiles.csv")
            )
            # print(f"Saved volatility quantiles data to: {os.path.join(results_dir, f'{file_prefix}_volatility_quantiles.csv')}")
        except (
            Exception
        ) as e:  # print(f"Error plotting/saving Volatility Quantiles: {e}"); traceback.print_exc();
            pass
        return results_vol_df

    def analyze_mean_returns(
        self,
        results_dir: str,
        file_prefix: str = "event",
        return_col: str = "ret",
        ...
        aligned_means = (
            ...
            .agg(
                [
                    ...
                    pl.median("rolling_mean_return_pct").alias(
                        "median_rolling_mean_return"
                    ),
                    pl.count().alias("event_count"),
                ]
            )
        ...
        ret_change_val = (
            (event_ret_val - baseline_ret_val)
        ...
        if not aligned_means.is_empty():
            try:
                ...
                fig, ax = plt.subplots(figsize=(12, 7))
                ax.plot(
                    aligned_means_pd["days_to_event"],
                    aligned_means_pd["avg_rolling_mean_return"],
                    color="blue",
                    linewidth=2,
                    label="Avg. Rolling Mean Return",
                )
                if baseline_ret_val is not None:
                    ax.axhline(
                        ...
                        color="green",
                        linestyle="--",
                        label=f"Baseline Ret: {baseline_ret_val:.2f}%",
                    )
                ...
                ax.set_title(
                    f"Average Rolling Mean Return Around Event (Window={window} days)"
                )
                ax.set_xlabel("Days Relative to Event")
    ...
    def calculate_mean_returns_quantiles(
        ...
        for day_val in days_range_list:
            ...
            if current_window_data.is_empty():
                empty_res_dict = {"days_to_event": day_val, "event_count": 0}
                [
                    empty_res_dict.update({f"mean_ret_q{int(q*100)}": None})
                    for q in [0.05, 0.5, 0.95]
                ]
                mean_ret_quantile_data.append(empty_res_dict)
                continue
            mean_ret_by_event_df = (
                current_window_data.group_by("event_id")
                .agg(
                    [
            ...
            if num_valid_events >= 5:
                mean_ret_by_event_df = mean_ret_by_event_df.with_columns(
                    (pl.col("mean_ret_raw") * 100).alias("mean_ret_pct")
                )
                quantile_values_dict = {
                    f"mean_ret_q{int(q*100)}": mean_ret_by_event_df.select(
                        pl.col("mean_ret_pct").quantile(q, interpolation="linear")
                    ).item()
                    for q in [0.05, 0.5, 0.95]
                }
                day_results_dict = {
                ...
                [
                    day_results_dict.update({f"mean_ret_q{int(q*100)}": None})
                    for q in [0.05, 0.5, 0.95]
                ]
            mean_ret_quantile_data.append(day_results_dict)
            del current_window_data, mean_ret_by_event_df
            gc.collect()
        ...
        try:
            ...
            for q_item in [0.05, 0.5, 0.95]:
                ...
                if col_name_str in results_mean_ret_pd.columns:
                    ax.plot(
                        results_mean_ret_pd["days_to_event"],
                        results_mean_ret_pd[col_name_str],
                        linewidth=(2 if q_item == 0.5 else 1),
                        linestyle=("solid" if q_item == 0.5 else "dashed"),
                        label=f"Q{int(q_item*100)}",
                    )
            ...
            ax.axvline(x=0, color="red", linestyle="--", label="Event Day")
            ax.set_title(
                f"Mean Return Quantiles Around Events (Lookback: {lookback_window} days)"
            )
            ax.set_xlabel("Days Relative to Event")
            ...
            plot_filename = os.path.join(
                results_dir, f"{file_prefix}_mean_return_quantiles.png"
            )
            plt.savefig(plot_filename, dpi=200, bbox_inches="tight")
            plt.close(fig)
            # print(f"Saved mean return quantiles plot to: {plot_filename}")
            results_mean_ret_df.write_csv(
                os.path.join(results_dir, f"{file_prefix}_mean_return_quantiles.csv")
            )
            # print(f"Saved mean return quantiles data to: {os.path.join(results_dir, f'{file_prefix}_mean_return_quantiles.csv')}")
        except (
            Exception
        ) as e:  # print(f"Error plotting/saving Mean Return Quantiles: {e}"); traceback.print_exc();
            pass
    ...
    def analyze_rvr(
        ...
        if (
            ...
            return rvr_daily_df
        if (
            post_event_delta <= 0 or post_event_delta > analysis_window[1]
        ...
        if (
            analysis_data_rvr.is_empty()
        ):  # print(f"Error: No data found within extended analysis window [{extended_start_day}, {analysis_window[1]}].");
            return rvr_daily_df
        sample_returns_rvr = analysis_data_rvr.select(pl.col("clipped_return")).sample(
        ...
        if adaptive_threshold:
            pct_05_rvr = (
                sample_returns_rvr.select(pl.col("clipped_return").quantile(0.05)).item(
                    0, 0
                )
                if sample_returns_rvr.height > 0 and sample_returns_rvr.width > 0
                else 0.0
            ...
            pct_95_rvr = (
                sample_returns_rvr.select(pl.col("clipped_return").quantile(0.95)).item(
                    0, 0
                )
                if sample_returns_rvr.height > 0 and sample_returns_rvr.width > 0
                else 0.0
            ...
            returns_in_pct_rvr = (
                range_check_rvr or std_check_rvr or (avg_abs_return_rvr > 0.05)
            )
        ...
        if returns_in_pct_rvr:
            ...
            analysis_data_rvr = analysis_data_rvr.rename(
                {"clipped_return": calc_return_col_rvr}
            )

        analysis_data_rvr = analysis_data_rvr.with_columns(
            [
                pl.col(calc_return_col_rvr)
                .rolling_mean(window_size=lookback_window, min_samples=min_samples)
                .over("event_id")
                .alias("mean_return_rvr"),
                pl.col(calc_return_col_rvr)
                .rolling_std(window_size=lookback_window, min_samples=min_samples)
                .over("event_id")
                .alias("volatility_rvr"),
            ]
        )
        adj_bias_rvr = optimistic_bias  # Already assumed to be decimal

        # FIXED: Apply smooth bias transition instead of step function
        def create_smooth_expected_returns_basic(
            days_series, mean_returns_series, bias_amount
        ):
            result = []
            for day, mean_ret in zip(
                days_series.to_list(), mean_returns_series.to_list()
            ):
        ...
        analysis_data_rvr = analysis_data_rvr.with_columns(
            ...
            pl.max_horizontal(
                pl.col("volatility_rvr") ** 2, pl.lit(variance_floor)
            ).alias("variance_rvr")
        )

        analysis_data_rvr = analysis_data_rvr.with_columns(
            pl.when(pl.col("variance_rvr") > 1e-9)
            .then(pl.col("expected_return_rvr") / pl.col("variance_rvr"))
            .otherwise(None)
            .alias("raw_rvr_calc")
        ).with_columns(
            pl.col("raw_rvr_calc")
            .clip(-rvr_clip_threshold, rvr_clip_threshold)
            .alias("rvr_final")
        )
        rvr_daily_df = (
            analysis_data_rvr.group_by("days_to_event")
            .agg(
                [
        ...
        for phase_name_str, (start_day_val, end_day_val) in phases_dict.items():
            phase_data_df = analysis_data_rvr.filter(
                (pl.col("days_to_event") >= start_day_val)
                & (pl.col("days_to_event") <= end_day_val)
            )
            if not phase_data_df.is_empty():
                phase_stats_dict = {
                    ...
                    "event_count": phase_data_df.filter(
                        pl.col("rvr_final").is_not_null()
                    ).height,
                }
            else:
                phase_stats_dict = {
        ...
        try:
            ...
            ax.axvline(
                ...
                color="purple",
                linestyle=":",
                label="End of Post-Event Rising",
            )
            ax.axvspan(
                phases_dict["post_event_rising"][0],
                phases_dict["post_event_rising"][1],
                color="yellow",
                alpha=0.2,
                label="Post-Event Rising",
            )
            # Auto y-limits for RVR plot
            valid_rvr_plot = rvr_pd_plot["avg_rvr"].dropna()
            y_lim_final_rvr = (
                [-abs(valid_rvr_plot).max() * 1.1, abs(valid_rvr_plot).max() * 1.1]
                if not valid_rvr_plot.empty
                else [-1, 1]
            )
            y_lim_final_rvr = [
                np.clip(y_lim_final_rvr[0], -200, 0),
                np.clip(y_lim_final_rvr[1], 0, 200),
            ]  # Reasonable RVR range
            if y_lim_final_rvr[0] == 0 and y_lim_final_rvr[1] == 0:
                y_lim_final_rvr = [-1, 1]  # if all zero
            ax.set_ylim(y_lim_final_rvr)

            ax.set_title(
                f"Return-to-Variance Ratio Around Events (Lookback: {lookback_window} days)"
            )
            ax.set_xlabel("Days Relative to Event")
            ...
            ax.grid(True, linestyle=":", alpha=0.7)
            plot_filename = os.path.join(
                results_dir, f"{file_prefix}_rvr_timeseries.png"
            )
            plt.savefig(plot_filename, dpi=200, bbox_inches="tight")
            plt.close(fig)
            # print(f"Saved RVR time series plot to: {plot_filename}")
            if rvr_daily_df is not None:
                rvr_daily_df.write_csv(
                    os.path.join(results_dir, f"{file_prefix}_rvr_daily.csv")
                )
            phase_summary_df.write_csv(
                os.path.join(results_dir, f"{file_prefix}_rvr_phase_summary.csv")
            )
        except (
            Exception
        ) as e:  # print(f"Error creating RVR plot: {e}"); traceback.print_exc();
            pass
        del analysis_data_rvr, phase_summary_df
        gc.collect()
        return rvr_daily_df

    def analyze_three_phase_volatility(
        self,
        results_dir: str,
        file_prefix: str = "event",
        return_col: str = "ret",
        ...
        if (
            proc_analysis_data.is_empty()
        ):  # print(f"Error: No data in extended window {extended_window}");
            return None
        ...
        sample_event_ids = (
            np.random.choice(event_ids, size=min(5, len(event_ids)), replace=False)
            if len(event_ids) > 0
            else []
        )
        ...
        for event_id in event_ids:
            event_data_current = proc_analysis_data.filter(
                pl.col("event_id") == event_id
            )
            event_days_current = event_data_current.get_column(
                "days_to_event"
            ).to_numpy()  # Corrected: get_column returns Series, then to_numpy
            event_returns_current = event_data_current.get_column(
                return_col
            )  # This is already a Series
            if len(event_returns_current) < 20:
                continue

            try:
                garch_model_current = (
                    GJRGARCHModel() if garch_type.lower() == "gjr" else GARCHModel()
                )
                ...
                vol_model_current = ThreePhaseVolatilityModel(
                    baseline_model=garch_model_current,
                    k1=k1,
                    k2=k2,
                    delta_t1=delta_t1,
                    delta_t2=delta_t2,
                    ...
                    delta=delta,
                    add_stochastic=True,
                    noise_std=0.03,  # Enable stochastic components
                )

                baseline_cond_vol_sqrt_h_t = np.sqrt(
                    garch_model_current.variance_history
                )
                day_to_baseline_vol_map = dict(
                    zip(event_days_current, baseline_cond_vol_sqrt_h_t)
                )

                bm = garch_model_current
                if isinstance(bm, GJRGARCHModel):
                    denom_uncond = 1 - bm.alpha - bm.beta - 0.5 * bm.gamma
                else:
                    denom_uncond = 1 - bm.alpha - bm.beta

                if denom_uncond > 1e-7:
                    fallback_uncond_vol = np.sqrt(max(bm.omega / denom_uncond, 1e-7))
                else:
                    fallback_uncond_vol = (
                        np.sqrt(bm.variance_history[-1])
                        if bm.variance_history is not None
                        and len(bm.variance_history) > 0
                        else np.sqrt(1e-6)
                    )

                aligned_baseline_cond_vol_series = np.array(
                    [
                        day_to_baseline_vol_map.get(day, fallback_uncond_vol)
                        for day in analysis_days_np
                    ]
                )

                predicted_vol_series = vol_model_current.calculate_volatility_series(
                    analysis_days_np, aligned_baseline_cond_vol_series
                )
                all_events_results.append(
                    {
                        "event_id": event_id,
                        "days_to_event": analysis_days_np,
                        "predicted_volatility": predicted_vol_series,
                    }
                )

                if event_id in sample_event_ids:
                    garch_vol_actual = np.sqrt(garch_model_current.variance_history)
                    aligned_garch_vol_for_sample = np.full_like(
                        event_days_current, np.nan, dtype=float
                    )
                    if len(garch_vol_actual) == len(event_days_current):
                        aligned_garch_vol_for_sample = garch_vol_actual
                    elif len(garch_vol_actual) > 0:
                        common_len = min(
                            len(garch_vol_actual), len(aligned_garch_vol_for_sample)
                        )
                        aligned_garch_vol_for_sample[:common_len] = garch_vol_actual[
                            :common_len
                        ]

                    sample_three_phase_vol = (
                        vol_model_current.calculate_volatility_series(
                            event_days_current, aligned_garch_vol_for_sample
                        )
                    )
                    sample_events_data.append(
                        {
                            "event_id": event_id,
                            "garch_days": event_days_current,
                            "garch_vol": aligned_garch_vol_for_sample,
                            "three_phase_days": event_days_current,
                            "three_phase_vol": sample_three_phase_vol,
                            "returns": event_returns_current.to_numpy()[
                                : len(aligned_garch_vol_for_sample)
                            ],
                            "ticker": event_data_current.get_column("ticker")
                            .head(1)
                            .item(),
                        }
                    )
                ...
                Exception
            ) as e:  # print(f"Error processing event {event_id} for 3-phase vol: {e}");
                pass  # traceback.print_exc()

        if (
            not all_events_results
        ):  # print("No valid 3-phase volatility results to analyze.");
            return None
        ...
        for phase_name, (start_day, end_day) in phases_def.items():
            phase_data_df = volatility_df_agg.filter(
                (pl.col("days_to_event") >= start_day)
                & (pl.col("days_to_event") <= end_day)
            )
            if not phase_data_df.is_empty():
                phase_stats_list.append(
                    {
                        "phase": phase_name,
                        "start_day": start_day,
                        "end_day": end_day,
                        "avg_volatility": phase_data_df["avg_volatility"].mean(),
                        "median_volatility": phase_data_df["median_volatility"].mean(),
        ...
        try:
            volatility_df_agg.write_csv(
                os.path.join(results_dir, f"{file_prefix}_three_phase_volatility.csv")
            )
            phase_stats_df_agg.write_csv(
                os.path.join(results_dir, f"{file_prefix}_volatility_phase_stats.csv")
            )
            ...
            fig, ax = plt.subplots(figsize=(12, 7))
            ax.plot(
                volatility_pd_plot["days_to_event"],
                volatility_pd_plot["avg_volatility"],
                color="blue",
                linewidth=2,
            ...
            ax.axvspan(
                0, delta, color="yellow", alpha=0.2, label="Post-Event Rising Phase"
            )
            ax.set_title(
                f"Three-Phase Volatility Around Events ({garch_type.upper()}-GARCH Model)"
            )
            ax.set_xlabel("Days Relative to Event")
            ax.set_ylabel("Volatility ( annualized std dev)")
            ax.legend(loc="best")
            ax.grid(True, linestyle=":", alpha=0.7)
            plt.tight_layout()
            plt.savefig(
                os.path.join(results_dir, f"{file_prefix}_three_phase_volatility.png"),
                dpi=200,
            )
            plt.close()
            ...
            for sample_data_item in sample_events_data:
                if (
                    sample_data_item["garch_vol"] is not None
                    and len(sample_data_item["garch_vol"]) > 0
                    and not np.all(np.isnan(sample_data_item["garch_vol"]))
                ):
                    fig_s, ax_s = plt.subplots(figsize=(12, 7))
                    ax_s.plot(
                        sample_data_item["garch_days"],
                        sample_data_item["garch_vol"] * np.sqrt(252),
                        color="blue",
                        linewidth=1,
                        alpha=0.7,
                        label="GARCH Volatility (annualized)",
                    )
                    ax_s.plot(
                        sample_data_item["three_phase_days"],
                        sample_data_item["three_phase_vol"] * np.sqrt(252),
                        color="red",
                        linewidth=2,
                        label="Three-Phase Model (annualized)",
                    )
                    ...
                    ax_s.set_title(
                        f"Volatility for {sample_data_item['ticker']} (Event ID: {sample_data_item['event_id']})"
                    )
                    ax_s.set_xlabel("Days Relative to Event")
                    ax_s.set_ylabel("Annualized Volatility")
                    ...
                    plt.savefig(
                        os.path.join(
                            results_dir,
                            f"{file_prefix}_volatility_sample_{sample_data_item['event_id']}.png",
                        ),
                        dpi=200,
                    )
                    plt.close()
            ...
            Exception
        ) as e:  # print(f"Error plotting/saving 3-phase volatility: {e}"); traceback.print_exc();
            pass
        return volatility_df_agg, phase_stats_df_agg

    def analyze_rvr_with_optimistic_bias(
        self,
        results_dir: str,
        file_prefix: str = "event",
        return_col: str = "ret",
        ...
        sample_returns = proc_analysis_data.select(pl.col(return_col)).sample(
        ...
        if (
            ...
            proc_analysis_data = proc_analysis_data.with_columns(
                (pl.col(return_col) / 100).alias(return_col_for_calc)
            )
            adj_bias = optimistic_bias / 100
        else:
            if return_col != return_col_for_calc:
                proc_analysis_data = proc_analysis_data.with_columns(
                    pl.col(return_col).alias(return_col_for_calc)
                )
        ...
        all_rvr_data_list = []
        analysis_days_np_rvr = np.array(
            range(analysis_window[0], analysis_window[1] + 1)
        )
        ...
        for event_id in event_ids:
            ...
            event_returns_for_garch_fit = event_data_current.get_column(
                return_col_for_calc
            )  # Already a Series

            if len(event_returns_for_garch_fit) < 20:
                continue
            ...
            try:
                ...
                event_days_of_fit = event_data_current.get_column(
                    "days_to_event"
                ).to_numpy()
                baseline_cond_vol_sqrt_h_t_series_fit = np.sqrt(
                    garch_model_current.variance_history
                )
                day_to_baseline_vol_map_fit = dict(
                    zip(event_days_of_fit, baseline_cond_vol_sqrt_h_t_series_fit)
                )
                ...
                if denom_uncond > 1e-7:
                    fallback_uncond_vol_fit = np.sqrt(
                        max(bm.omega / denom_uncond, 1e-7)
                    )
                else:
                    fallback_uncond_vol_fit = (
                        np.sqrt(bm.variance_history[-1])
                        if bm.variance_history is not None
                        and len(bm.variance_history) > 0
                        else np.sqrt(1e-6)
                ...
                aligned_baseline_cond_vol_for_analysis = np.array(
                    [
                        day_to_baseline_vol_map_fit.get(day, fallback_uncond_vol_fit)
                        for day in analysis_days_np_rvr
                    ]
                )
                volatility_series_for_rvr_analysis_days = (
                    vol_model_current.calculate_volatility_series(
                        analysis_days_np_rvr, aligned_baseline_cond_vol_for_analysis
                    )
                )
                ...
                mean_hist_return_event_analysis_win = (
                    event_returns_in_analysis_window_series.mean()
                    if not event_returns_in_analysis_window_series.is_empty()
                    else 0.0
                )
                if mean_hist_return_event_analysis_win is None:
                    mean_hist_return_event_analysis_win = 0.0  # handle all null case

                # FIXED: Apply smooth bias transition instead of step function
                expected_returns_biased_analysis_days = np.array(
                    [
                        mean_hist_return_event_analysis_win
                        + (adj_bias * self.smooth_bias_transition(day_val))
                        - risk_free_rate
                        for day_val in analysis_days_np_rvr
                    ]
                )

                rvr_values_analysis_days = expected_returns_biased_analysis_days / (
                    volatility_series_for_rvr_analysis_days**2 + 1e-10
                )
                    ...
                    all_rvr_data_list.append(
                        {
                            "event_id": event_id,
                            "days_to_event": day_val,
                            "expected_return_biased": expected_returns_biased_analysis_days[
                                idx
                            ],
                            "three_phase_volatility": volatility_series_for_rvr_analysis_days[
                                idx
                            ],
                ...
                Exception
            ) as e:  # print(f"Error processing event {event_id} for RVR: {e}");
                pass  # traceback.print_exc()
        ...
        agg_rvr_df = (
            combined_rvr_df.group_by("days_to_event")
            .agg(
                [
                    pl.mean("expected_return_biased").alias(
                        "mean_expected_return_biased"
                    ),
                    pl.mean("three_phase_volatility").alias(
                        "mean_three_phase_volatility"
                    ),
        ...
        for phase_name, (start_day, end_day) in phases.items():
            phase_data_rvr = agg_rvr_df.filter(
                (pl.col("days_to_event") >= start_day)
                & (pl.col("days_to_event") <= end_day)
            )
                ...
                phase_stats_rvr_list.append(
                    {
                        ...
                        "start_day": start_day,
                        "end_day": end_day,
                        "avg_rvr": phase_data_rvr["mean_rvr"].mean(),
                        "median_rvr": phase_data_rvr["median_rvr"].mean(),
                        "avg_volatility": phase_data_rvr[
                            "mean_three_phase_volatility"
                        ].mean(),
                        "avg_expected_return": phase_data_rvr[
                            "mean_expected_return_biased"
                        ].mean(),
        ...
        h1_test_data = {}
        for row_dict in phase_stats_rvr_df.filter(
            pl.col("phase").is_in(phases_to_highlight)
        ).to_dicts():
        ...
        h1_result = False
        if all(
            k in h1_test_data and h1_test_data[k] is not None
            for k in ["post_event_rising", "pre_event", "post_event_decay"]
        ):
            h1_result = (
                h1_test_data["post_event_rising"] > h1_test_data["pre_event"]
                and h1_test_data["post_event_rising"] > h1_test_data["post_event_decay"]
            )
        # print(f"\nHypothesis 1 (RVR peaks in post_event_rising): {'SUPPORTED' if h1_result else 'NOT SUPPORTED'}")

        try:
            agg_rvr_df.write_csv(
                os.path.join(results_dir, f"{file_prefix}_rvr_bias_timeseries.csv")
            )
            phase_stats_rvr_df.write_csv(
                os.path.join(results_dir, f"{file_prefix}_rvr_bias_phase_stats.csv")
            )
            h1_df = pl.DataFrame(
                {
            ...
            h1_df.write_csv(
                os.path.join(results_dir, f"{file_prefix}_hypothesis1_test.csv")
            )
            # print(f"Saved RVR (bias) analysis to {results_dir}")

            # Plotting
            rvr_pd_plot = agg_rvr_df.to_pandas()
            fig, ax = plt.subplots(figsize=(12, 7))
            ax.plot(
                rvr_pd_plot["days_to_event"],
                rvr_pd_plot["mean_rvr"],
                color="blue",
                linewidth=2,
                label="Mean RVR (Optimistic Bias)",
            )
            ax.axvline(x=0, color="red", linestyle="--", label="Event Day")
            ax.axvline(
                x=delta,
            ...
            ax.axvspan(
                phases["post_event_rising"][0],
                phases["post_event_rising"][1],
                color="yellow",
                alpha=0.2,
                label="Post-Event Rising Phase",
            )
            ...
            plt.tight_layout()
            plt.savefig(
                os.path.join(results_dir, f"{file_prefix}_rvr_bias_timeseries.png"),
                dpi=200,
            )
            plt.close()
            ...
            Exception
        ) as e:  # print(f"Error saving/plotting RVR (bias) analysis: {e}"); traceback.print_exc();
            pass
    ...
    def analyze_sharpe_ratio(
            ...
            if event_sharpe.height > 0:
                event_sharpe = event_sharpe.with_columns(
                    (
                        (pl.col("mean_return") - daily_rf)
                        / pl.col("std_return")
                        * (np.sqrt(252) if risk_free_rate > 0 else 1)
                    ).alias("sharpe_ratio")
                ).with_columns(
                    pl.col("sharpe_ratio")
                    .clip_quantile(0.05, 0.95)
        ...
        try:
            ...
            ax.set_xlabel("Days Relative to Event")
            ax.set_ylabel(
                "Sharpe Ratio" + (" (Annualized)" if risk_free_rate > 0 else "")
            )
            ax.legend()
            ax.grid(True, linestyle=":", alpha=0.7)
            plot_filename = os.path.join(
    ...
    def analyze_sharpe_ratio_quantiles(
        self,
        results_dir: str,
        file_prefix: str = "event",
        return_col: str = "ret",
        ...
        for day in days_range:
            ...
            if window_data_for_day.is_empty():
                empty_res = {"days_to_event": day, "event_count": 0}
                [
                    empty_res.update({f"sharpe_q{int(q*100)}": None})
                    for q in [0.05, 0.5, 0.95]
                ]
                sharpe_data.append(empty_res)
                continue
            ...
            if valid_events_count >= 5:
                ...
                day_res = {"days_to_event": day, "event_count": valid_events_count}
                [
                    day_res.update({f"sharpe_q{int(q*100)}": None})
                    for q in [0.05, 0.5, 0.95]
                ]
            sharpe_data.append(day_res)
            del window_data_for_day, sharpe_by_event_df
    ...
    def analyze_volatility_quantiles(
        self,
        results_dir: str,
        file_prefix: str = "event",
        return_col: str = "ret",
    ...
    def analyze_mean_return_quantiles(
        self,
        results_dir: str,
        file_prefix: str = "event",
        return_col: str = "ret",
