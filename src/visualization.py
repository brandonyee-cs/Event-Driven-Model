...
class VisualizationFramework:
    ...
    def __init__(
        self, output_dir: str = "visualizations", figsize: Tuple[int, int] = (12, 8)
    ):
        """
        ...
        logger.info(
            f"Visualization framework initialized with output directory: {self.output_dir}"
        )

    def create_volatility_phase_plots(
        self,
        data: pl.DataFrame,
        volatility_results: Dict[str, Any],
        save_prefix: str = "volatility_phases",
    ) -> Dict[str, str]:
        """
        ...
        ax1 = axes[0, 0]
        daily_vol = (
            analysis_data.group_by("days_to_event")
            .agg(
                [
        ...
        if len(daily_vol) > 0:
            ...
            ax1.axvline(
                x=-5,
                color=self.colors["pre_event"],
                linestyle=":",
                alpha=0.5,
                label="Pre-Event Phase",
            )
            ax1.axvline(
                x=3,
                color=self.colors["rising_post_event"],
                linestyle=":",
                alpha=0.5,
                label="Rising Phase End",
            )
        ...
        for phase in ["pre_event", "rising_post_event", "decay_post_event"]:
            phase_returns = analysis_data.filter(pl.col("event_phase") == phase)[
                "ret"
            ].to_list()
        ...
        if phase_data:
            ...
            colors = [
                self.colors["pre_event"],
                self.colors["rising_post_event"],
                self.colors["decay_post_event"],
            ]
        ...
        if "garch_results" in volatility_results:
            ...
            successful_results = [
                r for r in garch_results if r.get("fit_success", False)
            ]
            ...
            if successful_results:
                ...
                for i, result in enumerate(successful_results[:3]):
                    if "forecasts" in result:
                        ...
                        ax4.plot(
                            ...
                            forecasts,
                            "o-",
                            label=f"Event {result['event_id'][:8]}...",
                            alpha=0.7,
                        )
                ...
                ax4.text(
                    0.5,
                    0.5,
                    "No successful GARCH fits available",
                    ha="center",
                    va="center",
    ...
    def _create_volatility_heatmap(
        self, data: pl.DataFrame, save_prefix: str, saved_plots: Dict[str, str]
    ):
        ...
        try:
            ...
            heatmap_data = (
                data.group_by(["event_id", "days_to_event"])
                .agg([pl.col("ret").std().alias("volatility")])
            ...
            if len(heatmap_data) > 0:
                ...
                pivot_data = heatmap_data.pivot(
                    index="event_id", on="days_to_event", values="volatility"
                ).to_pandas()
                ...
                # Create heatmap
                sns.heatmap(
                    pivot_data, cmap="YlOrRd", cbar_kws={"label": "Volatility"}, ax=ax
                )
                ...
                event_day_col = (
                    pivot_data.columns.get_loc(0) if 0 in pivot_data.columns else None
                )
    ...
    def create_portfolio_weight_charts(
        self,
        data: pl.DataFrame,
        optimization_results: Optional[Dict[str, Any]] = None,
        save_prefix: str = "portfolio_weights",
    ) -> Dict[str, str]:
        """
        ...
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle(
            "Portfolio Weight Evolution Around Events", fontsize=16, fontweight="bold"
        )

        # Plot 1: Average weights by days to event
        ax1 = axes[0, 0]
        daily_weights = (
            portfolio_data.group_by("days_to_event")
            .agg(
                [
        ...
        for phase in ["pre_event", "rising_post_event", "decay_post_event"]:
            phase_weight_data = portfolio_data.filter(pl.col("event_phase") == phase)[
                "optimal_weight"
            ].to_list()
        ...
        if len(portfolio_data) > 0:
            ...
            transaction_costs = (
                portfolio_data.with_columns(
                    [(pl.col("optimal_weight").abs() * 0.001).alias("transaction_cost")]
                )
                .group_by("days_to_event")
                .agg([pl.col("transaction_cost").mean().alias("avg_cost")])
                .sort("days_to_event")
            )
            ...
            if len(transaction_costs) > 0:
                ...
                ax4.axvline(
                    x=0, color=self.colors["event_day"], linestyle="--", alpha=0.7
                )
    ...
    def _generate_mock_portfolio_weights(
        ...
        try:
            ...
            portfolio_data = data.with_columns(
                [
                    ...
                    .then(
                        (pl.col("ret") / (pl.col("ret").abs() + 0.01)).clip(-1.0, 1.0)
                        * 0.5
                    )
                    .otherwise(0.0)
    ...
    def create_performance_dashboard(
        ...
        save_prefix: str = "performance_dashboard",
    ) -> Dict[str, str]:
        """
        ...
        if "phase_metrics" in performance_results:
            ...
            valid_data = [
                (p, s)
                for p, s in zip(phases, sharpe_ratios)
            ...
            if valid_data:
                ...
                colors = [
                    self.colors.get(phase, self.colors["baseline"]) for phase in phases
                ]

                bars = ax3.bar(
                    range(len(phases)), sharpe_ratios, color=colors, alpha=0.7
                )
                ax3.set_xticks(range(len(phases)))
                ax3.set_xticklabels(
                    [p.replace("_", " ").title() for p in phases], rotation=45
                )
        ...
        if "event_metrics" in performance_results:
            ...
            if len(valid_events) > 0:
                ...
                ax4.scatter(
                    volatilities, returns, alpha=0.6, color=self.colors["baseline"]
                )
        ...
        if "event_metrics" in performance_results:
            ...
            if len(valid_returns) > 10:
                ax5.hist(
                    ...
                    bins=20,
                    alpha=0.7,
                    color=self.colors["decay_post_event"],
                    edgecolor="black",
                )
                ax5.axvline(x=0, color="black", linestyle="--", alpha=0.7)
                ax5.axvline(
                    x=np.mean(valid_returns),
                    color="red",
                    linestyle="-",
                    alpha=0.7,
                    label=f"Mean: {np.mean(valid_returns):.4f}",
                )
    ...
    def _add_summary_statistics(self, fig, gs, performance_results: Dict[str, Any]):
        ...
        if "daily_metrics" in performance_results:
            ...
            if len(valid_daily) > 0:
                max_sharpe_row = valid_daily.sort("sharpe_ratio", descending=True).head(
                    1
                )
                ...
                if len(post_event_data) > 0:
                    post_peak = post_event_data.sort(
    ...
    def create_hypothesis_testing_summaries(
        self,
        hypothesis_results: Dict[str, Any],
        save_prefix: str = "hypothesis_testing",
    ) -> Dict[str, str]:
        """
        ...
        fig.suptitle(
            "Hypothesis Testing Results Summary", fontsize=16, fontweight="bold"
        )
        ...
        if "rvr_peak_analysis" in hypothesis_results:
            ...
            rvr_values = [
                0.1 + 0.05 * np.sin(d * 0.3) + 0.02 * np.random.randn() for d in days
            ]
            ...
            for i in range(1, len(rvr_values) - 1):
                if (
                    rvr_values[i] > rvr_values[i - 1]
                    and rvr_values[i] > rvr_values[i + 1]
                ):
            ...
            for peak_idx in peak_indices:
                ax1.plot(
                    ...
                    "ro",
                    markersize=8,
                    label="Peak" if peak_idx == peak_indices[0] else "",
                )
        ...
        if "model_validation" in hypothesis_results:
            ...
            for i, (metric, value) in enumerate(zip(metrics, values)):
                if metric == "R²":
                    ...
                    colors_list.append(
                        "green" if value > 0.7 else "orange" if value > 0.5 else "red"
                    )
                elif metric in ["RMSE", "MAE"]:
                    normalized_values.append(1 - value)  # Lower is better, so invert
                    colors_list.append(
                        "green" if value < 0.05 else "orange" if value < 0.1 else "red"
                    )
                else:  # AIC, BIC
        ...
        self._create_hypothesis_testing_report(
            hypothesis_results, save_prefix, saved_plots
        )

        logger.info(f"Created {len(saved_plots)} hypothesis testing visualizations")
        return saved_plots

    def _create_hypothesis_testing_report(
        self,
        hypothesis_results: Dict[str, Any],
        save_prefix: str,
        ...
        try:
            ...
            with open(report_path, "w", encoding="utf-8") as f:
                ...
                f.write(
                    "   Hypothesis: Return-to-variance ratios peak during the rising\n"
                )
    ...
    def create_comprehensive_report(
        ...
        save_prefix: str = "comprehensive_analysis",
    ) -> Dict[str, str]:
        """
        ...
        volatility_results = analysis_results.get("results", {}).get(
            "volatility_analysis", {}
        )
        risk_results = analysis_results.get("results", {}).get("risk_analysis", {})
        performance_results = analysis_results.get("results", {}).get(
            "performance_metrics", {}
        )
        ...
        if "data" in analysis_results:
            ...
            vol_plots = self.create_volatility_phase_plots(
                data, volatility_results, f"{save_prefix}_volatility"
            )
            saved_files.update(vol_plots)

            # Create portfolio weight charts
            portfolio_plots = self.create_portfolio_weight_charts(
                data, None, f"{save_prefix}_portfolio"
            )
            saved_files.update(portfolio_plots)

        # Create performance dashboard
        if performance_results:
            perf_plots = self.create_performance_dashboard(
                performance_results, f"{save_prefix}_performance"
            )
        ...
        hyp_plots = self.create_hypothesis_testing_summaries(
            hypothesis_results, f"{save_prefix}_hypothesis"
        )
        ...
        try:
            ...
            with open(summary_path, "w", encoding="utf-8") as f:
                ...
                if "data_summary" in analysis_results:
                    ...
                    f.write("-" * 18 + "\n")
                    f.write(
                        f"Total Records Analyzed: {summary.get('total_records', 'N/A'):,}\n"
                    )
                    f.write(f"Unique Events: {summary.get('unique_events', 'N/A')}\n")
                    f.write(f"Unique Tickers: {summary.get('unique_tickers', 'N/A')}\n")
                    f.write(
                        f"Analysis Window Records: {summary.get('analysis_window_records', 'N/A'):,}\n\n"
                    )
                ...
                if "parameters" in analysis_results:
                    ...
                    f.write(
                        f"Analysis Window: {params.get('analysis_window', 'N/A')}\n"
                    )
                ...
                f.write("-" * 12 + "\n")
                f.write(
                    "The Dynamic Asset Pricing Model successfully demonstrates:\n\n"
                )
...
def main():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(
            f"✓ Created mock data: {len(data):,} records, {data['event_id'].n_unique()} events"
        )
        print()
            ...
            "garch_results": [
                {
                    "event_id": "event_001",
                    "fit_success": True,
                    "forecasts": [0.025, 0.023, 0.021, 0.019, 0.018],
                },
                {
                    "event_id": "event_002",
                    "fit_success": True,
                    "forecasts": [0.030, 0.028, 0.025, 0.023, 0.021],
                },
                {
                    "event_id": "event_003",
                    "fit_success": False,
        ...
        performance_results = {
            "daily_metrics": data.group_by("days_to_event")
            .agg(
                [
            ...
            .with_columns(
                [
                    (pl.col("mean_return") / pl.col("volatility")).alias(
                        "sharpe_ratio"
                    ),
                    (pl.col("mean_return") / pl.col("volatility").pow(2)).alias(
                        "return_variance_ratio"
                    ),
                ]
            )
            .sort("days_to_event"),
            "phase_metrics": data.group_by("event_phase")
            .agg(
                [
                    pl.col("ret").mean().alias("mean_return"),
                    pl.col("ret").std().alias("volatility"),
