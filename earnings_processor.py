...
class DataLoader:
    # ... (DataLoader class remains unchanged from previous version) ...
    def __init__(self, earnings_path: str, stock_paths: List[str], window_days: int = 30):
        ...
        self.event_chunk_size = 5000

    def _load_single_stock_parquet(self, stock_path: str) -> Optional[pl.DataFrame]:
        try:
            stock_data = pl.read_parquet(stock_path)
            original_columns = stock_data.columns; col_map_lower = {col.lower(): col for col in original_columns}
            standard_names = { 'date': ['date', 'trade_date', 'trading_date', 'tradedate', 'dt'], 'ticker': ['ticker', 'symbol', 'sym_root', 'tic', 'permno'], 'prc': ['prc', 'price', 'close', 'adj close', 'adj_prc'], 'ret': ['ret', 'return', 'daily_ret'], 'vol': ['vol', 'volume'], 'openprc': ['openprc', 'open'], 'askhi': ['askhi', 'high', 'askhigh'], 'bidlo': ['bidlo', 'low', 'bidlow'], 'shrout': ['shrout', 'shares_outstanding']}
            rename_dict = {}; found_std_names = {}; selected_cols = []
            for std_name, variations in standard_names.items():
                found = False;
                for var in variations:
                    if var in col_map_lower: original_case_col = col_map_lower[var];
                    if original_case_col != std_name: rename_dict[original_case_col] = std_name
                    found_std_names[std_name] = True; selected_cols.append(original_case_col); found = True; break
                if not found: found_std_names[std_name] = False
            essential_cols_found = all(found_std_names.get(c, False) for c in ['date', 'ticker', 'prc', 'ret']);
            if not essential_cols_found: return None
            stock_data = stock_data.select(selected_cols);
            if rename_dict: stock_data = stock_data.rename(rename_dict)
            required_cols = ['date', 'ticker', 'prc', 'ret', 'vol']; missing_cols = [col for col in required_cols if col not in stock_data.columns]
            if missing_cols: essential_pr = ['prc', 'ret'];
            if any(col in missing_cols for col in essential_pr): raise ValueError(f"Essential columns {missing_cols} missing in {stock_path} AFTER selection/rename.")
            if stock_data["date"].dtype != pl.Datetime: stock_data = stock_data.with_columns(pl.col("date").str.to_datetime(strict=False).alias("date"))
            n_null_dates = stock_data.filter(pl.col("date").is_null()).height;
            if n_null_dates > 0: stock_data = stock_data.drop_nulls(subset=["date"])
            if stock_data["ticker"].dtype != pl.Utf8: stock_data = stock_data.with_columns(pl.col("ticker").cast(pl.Utf8))
            numeric_cols_to_check = ['prc', 'ret', 'vol', 'openprc', 'askhi', 'bidlo', 'shrout']; cast_expressions = []
            for col in numeric_cols_to_check:
                if col in stock_data.columns:
                    cast_expressions.append(pl.when(pl.col(col).cast(pl.Float64, strict=False).is_infinite()).then(None).otherwise(pl.col(col).cast(pl.Float64, strict=False)).alias(col))
            if cast_expressions: stock_data = stock_data.with_columns(cast_expressions)
            final_cols = list(standard_names.keys()); cols_present = [col for col in final_cols if col in stock_data.columns]; stock_data = stock_data.select(cols_present); return stock_data
        except FileNotFoundError: print(f"Warning: Parquet file not found: {stock_path}"); return None
        except Exception as e: print(f"Warning: Error processing Parquet file {stock_path}: {e}"); return None
    ...
    def load_data(self) -> Optional[pl.DataFrame]:
        try:
            ...
            ticker_col = next((c for c in ['TICKER', 'ticker', 'Ticker', 'symbol', 'tic'] if c in event_df_peek.columns), None); date_col = 'ANNDATS'
            ...
            if event_data_renamed['Announcement Date'].dtype == pl.Object or isinstance(event_data_renamed['Announcement Date'].dtype, pl.String): event_data_processed = event_data_renamed.with_columns([pl.col('Announcement Date').str.to_datetime(strict=False).cast(pl.Datetime), pl.col('ticker').cast(pl.Utf8).str.to_uppercase()])
            elif isinstance(event_data_renamed['Announcement Date'].dtype, (pl.Date, pl.Datetime)): event_data_processed = event_data_renamed.with_columns([pl.col('Announcement Date').cast(pl.Datetime), pl.col('ticker').cast(pl.Utf8).str.to_uppercase()])
            else: raise TypeError(f"Unexpected dtype for 'Announcement Date': {event_data_renamed['Announcement Date'].dtype}")
            event_data_processed = event_data_processed.drop_nulls(subset=['Announcement Date']); earnings_events = event_data_processed.unique(subset=['ticker', 'Announcement Date'], keep='first'); n_total_events = earnings_events.height
            print("\n--- Sample Parsed Earnings Events ---"); print(earnings_events.head(5)); print("-" * 35 + "\n")
        ...
        processed_chunks = []; num_chunks = (n_total_events + self.event_chunk_size - 1) // self.event_chunk_size; print(f"Processing events in {num_chunks} chunk(s) of size {self.event_chunk_size}...")
        for i in range(num_chunks):
            start_idx = i * self.event_chunk_size; end_idx = min((i + 1) * self.event_chunk_size, n_total_events); event_chunk = earnings_events.slice(start_idx, end_idx - start_idx); print(f"--- Processing event chunk {i+1}/{num_chunks} ({event_chunk.height} events) ---")
            chunk_tickers = event_chunk['ticker'].unique(); min_ann_date = event_chunk['Announcement Date'].min(); max_ann_date = event_chunk['Announcement Date'].max(); buffer = pl.duration(days=self.window_days + 1); required_min_date = min_ann_date - buffer; required_max_date = max_ann_date + buffer
            print(f"    Chunk Tickers: {chunk_tickers.len()} (Sample: {chunk_tickers[:5].to_list()})"); print(f"    Required Stock Date Range: {required_min_date} to {required_max_date}")
            stock_scans = []; failed_stock_loads = 0; print("    Scanning and filtering stock Parquet files (lazily)...")
            for stock_path in self.stock_paths:
                try:
                    scan = pl.scan_parquet(stock_path); original_columns = list(scan.schema.keys()); col_map_lower = {col.lower(): col for col in original_columns}
                    standard_names = { 'date': ['date', 'trade_date', 'trading_date', 'tradedate', 'dt'], 'ticker': ['ticker', 'symbol', 'sym_root', 'tic', 'permno'], 'prc': ['prc', 'price', 'close', 'adj close', 'adj_prc'], 'ret': ['ret', 'return', 'daily_ret'], 'vol': ['vol', 'volume'], 'openprc': ['openprc', 'open'], 'askhi': ['askhi', 'high', 'askhigh'], 'bidlo': ['bidlo', 'low', 'bidlow'], 'shrout': ['shrout', 'shares_outstanding']}
                    rename_dict = {}; selected_orig_cols = []
                    for std_name, variations in standard_names.items():
                        for var in variations:
                            if var in col_map_lower: original_case_col = col_map_lower[var];
                            if original_case_col != std_name: rename_dict[original_case_col] = std_name
                            selected_orig_cols.append(original_case_col); break
                    if not any(sc in ['date','trade_date', 'trading_date', 'tradedate', 'dt'] for sc in col_map_lower) or not any(sc in ['ticker','symbol', 'sym_root', 'tic', 'permno'] for sc in col_map_lower): print(f"    Warning: Skipping {stock_path} as essential date or ticker column mapping not found."); continue
                    scan = scan.select(selected_orig_cols);
                    if rename_dict: scan = scan.rename(rename_dict)
                    date_dtype = scan.schema.get('date'); ticker_dtype = scan.schema.get('ticker')
                    if date_dtype is None or ticker_dtype is None: print(f"    Warning: 'date' or 'ticker' column missing in {stock_path} schema after selection/rename. Skipping."); continue
                    scan_expressions = []
                    if date_dtype == pl.Object or isinstance(date_dtype, pl.String): scan_expressions.append(pl.col("date").str.to_datetime(strict=False).cast(pl.Datetime))
                    else: scan_expressions.append(pl.col("date").cast(pl.Datetime))
                    if ticker_dtype != pl.Utf8: scan_expressions.append(pl.col("ticker").cast(pl.Utf8).str.to_uppercase())
                    else: scan_expressions.append(pl.col("ticker").str.to_uppercase())
                    scan = scan.with_columns(scan_expressions)
                    filtered_scan = scan.filter(pl.col('ticker').is_in(chunk_tickers) & pl.col('date').is_between(required_min_date, required_max_date, closed='both')); stock_scans.append(filtered_scan)
                except Exception as e: print(f"    Warning: Failed to scan/prepare stock file {stock_path}: {e}"); failed_stock_loads += 1
            if not stock_scans: print(f"    ERROR: No stock data could be scanned for chunk {i+1}. Skipping chunk."); continue
            combined_stock_scan = pl.concat(stock_scans, how='vertical_relaxed'); print(f"    Collecting filtered stock data for chunk {i+1}...")
            try: stock_data_chunk = combined_stock_scan.collect(streaming=True)
            except Exception as e: print(f"    ERROR collecting stock data for chunk {i+1}: {e}. Skipping chunk."); traceback.print_exc(); continue
            print(f"    Collected {stock_data_chunk.height} stock rows.")
            if stock_data_chunk.is_empty(): print("    Collected stock data is empty. Skipping rest of chunk processing."); continue
            cast_expressions_collect = []; numeric_cols_to_check = ['prc', 'ret', 'vol', 'openprc', 'askhi', 'bidlo', 'shrout']
            for col in numeric_cols_to_check:
                if col in stock_data_chunk.columns: cast_expressions_collect.append(pl.when(pl.col(col).cast(pl.Float64, strict=False).is_infinite()).then(None).otherwise(pl.col(col).cast(pl.Float64, strict=False)).alias(col))
            if cast_expressions_collect: stock_data_chunk = stock_data_chunk.with_columns(cast_expressions_collect)
            stock_data_chunk = stock_data_chunk.with_columns([pl.col('ticker').cast(pl.Utf8), pl.col('date').cast(pl.Datetime)])
            stock_data_chunk = stock_data_chunk.unique(subset=['date', 'ticker'], keep='first', maintain_order=False); print(f"    Deduplicated stock rows: {stock_data_chunk.height}")
            if stock_data_chunk.is_empty(): print(f"    Warning: No stock data remained after deduplication for chunk {i+1}. Skipping chunk."); continue
            print(f"    Merging events with stock data..."); event_chunk = event_chunk.with_columns([pl.col('ticker').cast(pl.Utf8), pl.col('Announcement Date').cast(pl.Datetime)])
            merged_chunk = stock_data_chunk.join(event_chunk, on='ticker', how='inner'); print(f"    Merged chunk rows: {merged_chunk.height}")
            if merged_chunk.is_empty(): print(f"    Warning: Merge resulted in empty data for chunk {i+1}. Check ticker matching."); continue
            processed_chunk = merged_chunk.with_columns((pl.col('date') - pl.col('Announcement Date')).dt.total_days().cast(pl.Int32).alias('days_to_announcement')).filter((pl.col('days_to_announcement') >= -self.window_days) & (pl.col('days_to_announcement') <= self.window_days))
            print(f"    Rows after window filter ({self.window_days} days): {processed_chunk.height}")
            if processed_chunk.is_empty(): print(f"    Warning: No data found within event window for chunk {i+1}."); continue
            processed_chunk = processed_chunk.with_columns([(pl.col('days_to_announcement') == 0).cast(pl.Int8).alias('is_announcement_date'), (pl.col("ticker") + "_" + pl.col("Announcement Date").dt.strftime('%Y%m%d')).alias('event_id')])
            stock_cols = stock_data_chunk.columns; event_cols = event_chunk.columns; derived_cols = ['days_to_announcement', 'is_announcement_date', 'event_id']; final_cols = list(set(stock_cols) | set(event_cols) | set(derived_cols))
            final_cols = [c for c in final_cols if c in processed_chunk.columns]; processed_chunk = processed_chunk.select(final_cols); print(f"    Processed chunk {i+1} FINAL shape: {processed_chunk.shape}"); processed_chunks.append(processed_chunk); print(f"--- Finished processing chunk {i+1} ---")
            del stock_data_chunk, merged_chunk, event_chunk, processed_chunk, combined_stock_scan, stock_scans; gc.collect()
        if not processed_chunks: print("Error: No data chunks were processed successfully."); return None
        print("\nConcatenating processed chunks..."); combined_data = pl.concat(processed_chunks, how='vertical').sort(['ticker', 'Announcement Date', 'date']); print(f"Final Earnings dataset shape: {combined_data.shape}"); mem_usage_mb = combined_data.estimated_size("mb"); print(f"Final DataFrame memory usage: {mem_usage_mb:.2f} MB")
        if combined_data.is_empty(): print("Warning: Final combined data is empty after chunk processing."); return None
        return combined_data


class FeatureEngineer:
    # ... (FeatureEngineer class remains unchanged) ...
    def __init__(self, prediction_window: int = 3):
        self.windows = [5, 10, 20]; self.prediction_window = prediction_window; self.imputer = SimpleImputer(strategy='median'); self.feature_names: List[str] = []; self.final_feature_names: List[str] = []; self.categorical_features: List[str] = []; self.sector_dummies_cols: Optional[List[str]] = None; self.industry_dummies_cols: Optional[List[str]] = None; self.top_industries: Optional[List[str]] = None; self._imputer_fitted = False
    def create_target(self, df: pl.DataFrame, price_col: str = 'prc') -> pl.DataFrame:
        ...
        df = df.sort(['event_id', 'date']); df = df.with_columns(pl.col(price_col).shift(-self.prediction_window).over('event_id').alias('future_price')).with_columns(pl.when(pl.col(price_col).is_not_null() & (pl.col(price_col) != 0)).then((pl.col('future_price') / pl.col(price_col)) - 1).otherwise(None).alias('future_ret')).drop('future_price'); print(f"'future_ret' created. Non-null: {df.filter(pl.col('future_ret').is_not_null()).height}"); return df
    def calculate_features(self, df: pl.DataFrame, price_col: str = 'prc', return_col: str = 'ret', volume_col: str = 'vol', fit_categorical: bool = False) -> pl.DataFrame:
        print("Calculating Earnings features (Polars)..."); required = ['event_id', price_col, return_col, 'Announcement Date', 'date', 'days_to_announcement']; missing = [col for col in required if col not in df.columns];
        ...
        df = df.sort(['event_id', 'date']); current_features: List[str] = []; self.categorical_features = []; feature_expressions = []
        for window in self.windows: col_name = f'momentum_{window}'; shifted_price = pl.col(price_col).shift(window).over('event_id'); feature_expressions.append(pl.when(shifted_price.is_not_null() & (shifted_price != 0)).then((pl.col(price_col) / shifted_price) - 1).otherwise(None).alias(col_name)); current_features.append(col_name)
        df = df.with_columns(feature_expressions); feature_expressions = []; feature_expressions.append((pl.col('momentum_5') - pl.col('momentum_10')).alias('delta_momentum_5_10')); current_features.append('delta_momentum_5_10'); feature_expressions.append((pl.col('momentum_10') - pl.col('momentum_20')).alias('delta_momentum_10_20')); current_features.append('delta_momentum_10_20')
        for window in self.windows: col_name = f'volatility_{window}'; min_p = max(2, min(window, 5)); feature_expressions.append(pl.col(return_col).rolling_std(window_size=window, min_periods=min_p).over('event_id').alias(col_name)); current_features.append(col_name)
        df = df.with_columns(feature_expressions); feature_expressions = []; feature_expressions.append((pl.col('volatility_5') - pl.col('volatility_10')).alias('delta_volatility_5_10')); current_features.append('delta_volatility_5_10'); feature_expressions.append((pl.col('volatility_10') - pl.col('volatility_20')).alias('delta_volatility_10_20')); current_features.append('delta_volatility_10_20')
        shifted_price_log = pl.col(price_col).shift(1).over('event_id'); feature_expressions.append(pl.when(shifted_price_log.is_not_null() & (shifted_price_log > 0) & pl.col(price_col).is_not_null() & (pl.col(price_col) > 0) ).then(pl.ln(pl.col(price_col) / shifted_price_log)).otherwise(None).alias('log_ret')); current_features.append('log_ret'); current_features.append('days_to_announcement')
        for lag in range(1, 4): col_name = f'ret_lag_{lag}'; feature_expressions.append(pl.col(return_col).shift(lag).over('event_id').alias(col_name)); current_features.append(col_name)
        df = df.with_columns(feature_expressions); feature_expressions = []
        if has_volume:
            vol_mean_20d = pl.col(volume_col).rolling_mean(window_size=20, min_periods=5).over('event_id'); df = df.with_columns(pl.when(vol_mean_20d.is_not_null() & (vol_mean_20d != 0)).then(pl.col(volume_col) / vol_mean_20d).otherwise(None).alias('norm_vol')); current_features.append('norm_vol')
            for window in [5, 10]: col_name = f'vol_momentum_{window}'; shifted_vol = pl.col(volume_col).shift(window).over('event_id'); df = df.with_columns(pl.when(shifted_vol.is_not_null() & (shifted_vol != 0)).then((pl.col(volume_col) / shifted_vol) - 1).otherwise(None).alias(col_name)); current_features.append(col_name)
        else: df = df.with_columns([pl.lit(None).cast(pl.Float64).alias(c) for c in ['norm_vol', 'vol_momentum_5', 'vol_momentum_10']]); current_features.extend(['norm_vol', 'vol_momentum_5', 'vol_momentum_10'])
        pre_announce_start_offset = pl.duration(days=-30); pre_announce_data = df.filter((pl.col('date') < pl.col('Announcement Date')) & (pl.col('date') >= (pl.col('Announcement Date') + pre_announce_start_offset))); pre_announce_agg = pre_announce_data.group_by('event_id').agg((pl.col(return_col).fill_null(0) + 1).product().alias("prod_ret_factor")).with_columns((pl.col("prod_ret_factor") - 1).alias('pre_announce_ret_30d')).select(['event_id', 'pre_announce_ret_30d']); df = df.join(pre_announce_agg, on='event_id', how='left'); df = df.with_columns(pl.col('pre_announce_ret_30d').fill_null(0)); current_features.append('pre_announce_ret_30d')
        surprise_cols_to_add = ['surprise_val', 'pos_surprise', 'neg_surprise', 'surprise_magnitude', 'prev_surprise', 'consecutive_beat', 'consecutive_miss']
        if 'Surprise' in df.columns:
            print("Info: 'Surprise' column found."); df = df.with_columns(pl.col('Surprise').cast(pl.Float64, strict=False).fill_null(0).alias('surprise_val')); df = df.with_columns([(pl.col('surprise_val') > 0).cast(pl.Int8).alias('pos_surprise'), (pl.col('surprise_val') < 0).cast(pl.Int8).alias('neg_surprise'), pl.col('surprise_val').abs().alias('surprise_magnitude'), pl.col('surprise_val').shift(1).over('ticker').fill_null(0).alias('prev_surprise')]); df = df.with_columns([((pl.col('surprise_val') > 0) & (pl.col('prev_surprise') > 0)).cast(pl.Int8).alias('consecutive_beat'), ((pl.col('surprise_val') < 0) & (pl.col('prev_surprise') < 0)).cast(pl.Int8).alias('consecutive_miss')]); current_features.extend(surprise_cols_to_add)
        else: print("Info: 'Surprise' column not present."); df = df.with_columns([pl.lit(None).cast(pl.Float64).alias(c) for c in surprise_cols_to_add if c not in ['pos_surprise', 'neg_surprise', 'consecutive_beat', 'consecutive_miss']] + [pl.lit(None).cast(pl.Int8).alias(c) for c in ['pos_surprise', 'neg_surprise', 'consecutive_beat', 'consecutive_miss']]); current_features.extend(surprise_cols_to_add)
        time_features = ['announcement_hour', 'is_bmo', 'is_amc', 'is_market_hours'];
        if 'Time' in df.columns:
            print("Info: 'Time' column found."); df = df.with_columns(pl.col('Time').str.to_time(strict=False, format="%H:%M:%S").alias('time_parsed'));
            if df['time_parsed'].is_null().all(): print("  Warning: Failed to parse 'Time'."); df = df.drop('time_parsed'); df = df.with_columns([pl.lit(None).cast(pl.Int8).alias(c) for c in time_features])
            else: df = df.with_columns(pl.col('time_parsed').dt.hour().fill_null(-1).cast(pl.Int8).alias('announcement_hour')).drop('time_parsed'); df = df.with_columns([((pl.col('announcement_hour') >= 0) & (pl.col('announcement_hour') < 9)).cast(pl.Int8).alias('is_bmo'), ((pl.col('announcement_hour') >= 16) & (pl.col('announcement_hour') <= 23)).cast(pl.Int8).alias('is_amc'), ((pl.col('announcement_hour') >= 9) & (pl.col('announcement_hour') < 16)).cast(pl.Int8).alias('is_market_hours')]); current_features.extend(time_features)
        else: print("Info: 'Time' column not present."); df = df.with_columns([pl.lit(None).cast(pl.Int8).alias(c) for c in time_features]); current_features.extend(time_features)
        quarter_features = ['quarter_num'] + [f'is_q{i}' for i in range(1, 5)];
        if 'Quarter' in df.columns:
            print("Info: 'Quarter' column found."); df = df.with_columns(pl.col('Quarter').cast(pl.Utf8).str.extract(r"Q(\d)", 1).cast(pl.Int8, strict=False).alias('quarter_num')); quarter_expressions = []
            for i in range(1, 5): col_name = f'is_q{i}'; quarter_expressions.append((pl.col('quarter_num') == i).fill_null(False).cast(pl.Int8).alias(col_name)); self.categorical_features.append(col_name)
            df = df.with_columns(quarter_expressions); current_features.extend(quarter_features)
        else: print("Info: 'Quarter' column not present."); df = df.with_columns([pl.lit(None).cast(pl.Int8).alias(c) for c in quarter_features]); current_features.extend(quarter_features)
        if 'Sector' in df.columns:
            print("Info: 'Sector' column found."); df = df.with_columns(pl.col('Sector').fill_null('Unknown').cast(pl.Utf8))
            if fit_categorical: sector_dummies_df = df.select('Sector').to_dummies(columns=['Sector'], drop_first=True); self.sector_dummies_cols = sector_dummies_df.columns; print(f"Learned {len(self.sector_dummies_cols)} sector dummies.")
            if self.sector_dummies_cols:
                current_sector_dummies = df.select('Sector').to_dummies(columns=['Sector'], drop_first=True); missing_dummies = set(self.sector_dummies_cols) - set(current_sector_dummies.columns);
                if missing_dummies: current_sector_dummies = current_sector_dummies.with_columns([pl.lit(0).cast(pl.UInt8).alias(col) for col in missing_dummies])
                if current_sector_dummies.width > 0: df = pl.concat([df, current_sector_dummies.select(self.sector_dummies_cols)], how="horizontal"); current_features.extend(self.sector_dummies_cols); self.categorical_features.extend(self.sector_dummies_cols)
                else: print("  Warning: No sector dummies generated.")
            elif fit_categorical: print("  Warning: Could not learn sector dummies.")
        else: print("Info: 'Sector' column not present.")
        if 'Industry' in df.columns:
            print("Info: 'Industry' column found."); df = df.with_columns(pl.col('Industry').fill_null('Unknown').cast(pl.Utf8))
            if fit_categorical:
                top_n = 20; industry_counts = df['Industry'].value_counts().sort(by="counts", descending=True); self.top_industries = industry_counts.head(top_n)['Industry'].to_list(); df_temp = df.with_columns(pl.when(pl.col('Industry').is_in(self.top_industries)).then(pl.col('Industry')).otherwise(pl.lit('Other_Industry')).alias('Industry_Top')); industry_dummies_df = df_temp.select('Industry_Top').to_dummies(columns=['Industry_Top'], drop_first=True); self.industry_dummies_cols = industry_dummies_df.columns; print(f"Learned {len(self.industry_dummies_cols)} industry dummies.")
            if self.industry_dummies_cols and self.top_industries:
                df = df.with_columns(pl.when(pl.col('Industry').is_in(self.top_industries)).then(pl.col('Industry')).otherwise(pl.lit('Other_Industry')).alias('Industry_Top')); current_industry_dummies = df.select('Industry_Top').to_dummies(columns=['Industry_Top'], drop_first=True); missing_dummies = set(self.industry_dummies_cols) - set(current_industry_dummies.columns);
                if missing_dummies: current_industry_dummies = current_industry_dummies.with_columns([pl.lit(0).cast(pl.UInt8).alias(col) for col in missing_dummies])
                if current_industry_dummies.width > 0: df = pl.concat([df, current_industry_dummies.select(self.industry_dummies_cols)], how="horizontal"); current_features.extend(self.industry_dummies_cols); self.categorical_features.extend(self.industry_dummies_cols)
                else: print("  Warning: No industry dummies generated.")
            elif fit_categorical: print("  Warning: Could not learn industry dummies.")
        else: print("Info: 'Industry' column not present.")
        feature_cols_in_df = [f for f in current_features if f in df.columns]; df = df.with_columns([pl.when(pl.col(c).is_infinite()).then(None).otherwise(pl.col(c)).alias(c) for c in feature_cols_in_df if df.select(pl.col(c)).dtypes[0] in [pl.Float32, pl.Float64]])
        self.feature_names = sorted(list(set(current_features))); print(f"Calculated/checked {len(self.feature_names)} raw Earnings features.")
        base_required = ['ticker', 'date', 'Announcement Date', 'ret', 'prc', 'days_to_announcement', 'event_id', 'future_ret']; optional_inputs = ['Sector', 'Industry', 'Time', 'Quarter', 'Surprise']; keep_cols = base_required + [c for c in optional_inputs if c in df.columns] + self.feature_names; final_cols_to_keep = sorted(list(set(c for c in keep_cols if c in df.columns))); return df.select(final_cols_to_keep)
    def get_features_target(self, df: pl.DataFrame, fit_imputer: bool = False) -> Tuple[np.ndarray, np.ndarray, List[str]]:
        ...
        df = df.with_columns([pl.when(pl.col(c).is_infinite()).then(None).otherwise(pl.col(c)).alias(c) for c in available_features if df.select(pl.col(c)).dtypes[0] in [pl.Float32, pl.Float64]])
        df_with_target = df.drop_nulls(subset=['future_ret'])
        numeric_features = [f for f in available_features if f not in self.categorical_features]; categorical_cols_present = [f for f in available_features if f in self.categorical_features]; num_cols_total = len(numeric_features) + len(categorical_cols_present)
        if df_with_target.is_empty(): print("Warning: No data remains after filtering for non-null target."); return np.array([]).reshape(0, num_cols_total), np.array([]), numeric_features + categorical_cols_present
        categorical_df = df_with_target.select(categorical_cols_present); X_numeric_pl = df_with_target.select(numeric_features); y_pl = df_with_target.get_column('future_ret')
        print(f"Original numeric X (Polars): {X_numeric_pl.shape}. Categorical X (Polars): {categorical_df.shape}. Non-null y: {y_pl.len()}")
        try: X_numeric_np = X_numeric_pl.select([pl.col(c).cast(pl.Float64, strict=False) for c in numeric_features]).to_numpy()
        ...
        if fit_imputer:
            ...
            if X_numeric_np.size > 0: self.imputer.fit(X_numeric_np); self._imputer_fitted = True; print("Transforming with imputer..."); X_numeric_imputed_np = self.imputer.transform(X_numeric_np)
            else: print("Warning: Numeric feature array is empty."); X_numeric_imputed_np = X_numeric_np; self._imputer_fitted = True
        else:
            if not self._imputer_fitted: raise RuntimeError("Imputer not fitted.")
            print("Transforming numeric NumPy data with pre-fitted imputer...");
            if X_numeric_np.size > 0: X_numeric_imputed_np = self.imputer.transform(X_numeric_np)
            else: X_numeric_imputed_np = X_numeric_np
        ...
        if categorical_df.width > 0:
            try: X_categorical_np = categorical_df.select([pl.col(c).cast(pl.UInt8, strict=False) for c in categorical_cols_present]).to_numpy()
            except Exception as e: raise ValueError(f"Failed to convert categorical features to NumPy: {e}")
        else: X_categorical_np = np.empty((X_numeric_imputed_np.shape[0], 0), dtype=np.uint8); print("No categorical features found/used.")
        ...
        if X_numeric_imputed_np.size > 0 and X_categorical_np.size > 0: X_np = np.concatenate([X_numeric_imputed_np, X_categorical_np], axis=1); self.final_feature_names = numeric_features + categorical_cols_present
        elif X_numeric_imputed_np.size > 0: X_np = X_numeric_imputed_np; self.final_feature_names = numeric_features
        elif X_categorical_np.size > 0: X_np = X_categorical_np; self.final_feature_names = categorical_cols_present
        else: X_np = np.empty((0, num_cols_total), dtype=np.float64); self.final_feature_names = numeric_features + categorical_cols_present
        y_np = y_pl.cast(pl.Float64).to_numpy(); print(f"Final X NumPy shape: {X_np.shape}. y NumPy shape: {y_np.shape}. Using {len(self.final_feature_names)} features.")
...
class SurpriseClassificationModel:
    ...
    def __init__(self, xgb_cls_params=None, xgb_reg_params=None):
        ...
        self.surprise_pos_model = xgb.XGBClassifier(**self.xgb_cls_params); self.surprise_neg_model = xgb.XGBClassifier(**self.xgb_cls_params); self.return_model = xgb.XGBRegressor(**self.xgb_reg_params); self.feature_names_in_ = None; self.return_feature_names_in_ = None
    def fit(self, X_np: np.ndarray, y_np: np.ndarray, surprise_np: np.ndarray, feature_names: List[str]):
        ...
        if not all(isinstance(arr, np.ndarray) for arr in [X_np, y_np, surprise_np]): raise TypeError("Inputs X_np, y_np, surprise_np must be NumPy arrays.")
        if not X_np.shape[0] == y_np.shape[0] == surprise_np.shape[0]: raise ValueError("Inputs X_np, y_np, surprise_np must have the same number of rows.")
        self.feature_names_in_ = feature_names; pos_surprise_target = (surprise_np > 0).astype(int); neg_surprise_target = (surprise_np < 0).astype(int); print("  Fitting positive/negative surprise classifiers...")
        if len(np.unique(pos_surprise_target)) > 1: self.surprise_pos_model.fit(X_np, pos_surprise_target)
        else: print("Warning: Positive surprise target is constant.")
        if len(np.unique(neg_surprise_target)) > 1: self.surprise_neg_model.fit(X_np, neg_surprise_target)
        else: print("Warning: Negative surprise target is constant.")
        X_df_temp = pl.DataFrame(X_np, schema=self.feature_names_in_, strict=False); X_with_surprise_pl = X_df_temp.with_columns([pl.Series('surprise_value_actual', surprise_np), pl.Series('pos_surprise_actual', pos_surprise_target), pl.Series('neg_surprise_actual', neg_surprise_target), pl.Series('surprise_magnitude_actual', np.abs(surprise_np))]); self.return_feature_names_in_ = X_with_surprise_pl.columns; X_with_surprise_np = X_with_surprise_pl.to_numpy(); print("  Fitting return regressor..."); self.return_model.fit(X_with_surprise_np, y_np); print("SurpriseClassificationModel fitting complete."); return self
    def predict(self, X_np: np.ndarray) -> Dict[str, np.ndarray]:
        ...
        if X_np.ndim != 2 or X_np.shape[1] != len(self.feature_names_in_): raise ValueError(f"Input X_np has shape {X_np.shape}, but model expected (n_samples, {len(self.feature_names_in_)}).")
        pos_prob = np.zeros(X_np.shape[0]); neg_prob = np.zeros(X_np.shape[0])
        if hasattr(self.surprise_pos_model, 'classes_'): pos_prob = self.surprise_pos_model.predict_proba(X_np)[:, 1]
        if hasattr(self.surprise_neg_model, 'classes_'): neg_prob = self.surprise_neg_model.predict_proba(X_np)[:, 1]
        pos_pred = (pos_prob > 0.5).astype(int); neg_pred = (neg_prob > 0.5).astype(int)
        X_df_temp = pl.DataFrame(X_np, schema=self.feature_names_in_, strict=False); X_for_ret_pl = X_df_temp.with_columns([pl.Series('surprise_value_actual', pos_prob - neg_prob), pl.Series('pos_surprise_actual', pos_pred), pl.Series('neg_surprise_actual', neg_pred), pl.Series('surprise_magnitude_actual', np.abs(pos_prob - neg_prob))]); missing_ret_cols = set(self.return_feature_names_in_) - set(X_for_ret_pl.columns);
        if missing_ret_cols: raise ValueError(f"Internal error: Missing columns required for return prediction: {missing_ret_cols}")
        X_for_ret_np = X_for_ret_pl.select(self.return_feature_names_in_).to_numpy(); return_pred = self.return_model.predict(X_for_ret_np); return {'pos_surprise_prob': pos_prob, 'neg_surprise_prob': neg_prob, 'pos_surprise_pred': pos_pred, 'neg_surprise_pred': neg_pred, 'return_pred': return_pred}
    def evaluate(self, X_np: np.ndarray, y_np: np.ndarray, surprise_np: np.ndarray) -> Dict[str, float]:
        if not all(isinstance(arr, np.ndarray) for arr in [X_np, y_np, surprise_np]): raise TypeError("Inputs X_np, y_np, surprise_np must be NumPy arrays.")
        if not X_np.shape[0] == y_np.shape[0] == surprise_np.shape[0]: raise ValueError("Inputs X_np, y_np, surprise_np must have the same number of rows.")
        if X_np.shape[0] == 0: print("Warning: Empty input for SurpriseClassificationModel evaluation."); return {'pos_surprise_accuracy': np.nan, 'neg_surprise_accuracy': np.nan, 'return_mse': np.nan, 'return_rmse': np.nan, 'return_r2': np.nan, 'return_direction_accuracy': np.nan}
        preds = self.predict(X_np); pos_actual = (surprise_np > 0).astype(int); neg_actual = (surprise_np < 0).astype(int); pos_acc = np.nan; neg_acc = np.nan; print("\n--- Surprise Classification Report (Positive) ---")
        if hasattr(self.surprise_pos_model, 'classes_'): print(classification_report(pos_actual, preds['pos_surprise_pred'], zero_division=0)); pos_acc = accuracy_score(pos_actual, preds['pos_surprise_pred'])
        else: print("Positive classifier was not fitted.")
        print("\n--- Surprise Classification Report (Negative) ---")
        if hasattr(self.surprise_neg_model, 'classes_'): print(classification_report(neg_actual, preds['neg_surprise_pred'], zero_division=0)); neg_acc = accuracy_score(neg_actual, preds['neg_surprise_pred'])
        else: print("Negative classifier was not fitted.")
        valid_mask = np.isfinite(y_np) & np.isfinite(preds['return_pred']); y_valid = y_np[valid_mask]; y_pred_valid = preds['return_pred'][valid_mask]; ret_mse, ret_r2, ret_dir_acc = np.nan, np.nan, np.nan
        if len(y_valid) > 0: ret_mse = mean_squared_error(y_valid, y_pred_valid); ret_r2 = r2_score(y_valid, y_pred_valid); ret_dir_acc = np.mean(np.sign(y_pred_valid) == np.sign(y_valid))
        print("\n--- Return Prediction Evaluation ---"); print(f"  MSE={ret_mse:.6f}, RMSE={np.sqrt(ret_mse):.6f}, R2={ret_r2:.4f}, DirAcc={ret_dir_acc:.4f}"); return {'pos_surprise_accuracy': pos_acc, 'neg_surprise_accuracy': neg_acc, 'return_mse': ret_mse, 'return_rmse': np.sqrt(ret_mse), 'return_r2': ret_r2, 'return_direction_accuracy': ret_dir_acc}
...
class EarningsDriftModel:
    ...
    def __init__(self, time_horizons: List[int] = [1, 3, 5, 10, 20], model_params: Optional[Dict] = None):
        ...
        if model_params is None: self.model_params = {'n_estimators': 100, 'max_depth': 4, 'learning_rate': 0.05, 'objective': 'reg:squarederror', 'subsample': 0.8, 'colsample_bytree': 0.8, 'random_state': 42, 'n_jobs': -1, 'eval_metric': 'rmse', 'use_label_encoder': False}
        else: self.model_params = {**model_params, 'use_label_encoder': False}
        self.models = {h: xgb.XGBRegressor(**self.model_params) for h in time_horizons}; self.feature_names_in_: Optional[List[str]] = None; self.imputers: Dict[int, SimpleImputer] = {}
    def _prepare_data_for_horizon(self, data: pl.DataFrame, horizon: int, feature_cols: List[str], return_col: str = 'ret') -> Tuple[Optional[pl.DataFrame], Optional[pl.Series]]:
        base_required = ['event_id', 'date', 'days_to_announcement', return_col, 'Announcement Date']; required = base_required + feature_cols;
        if not all(c in data.columns for c in required): missing = [c for c in required if c not in data.columns]; print(f"Warning (PEAD Prep): Missing columns for horizon {horizon}: {missing}. Skipping."); return None, None
        data = data.sort(['event_id', 'date']); data = data.with_columns((pl.col(return_col).shift(-1).over("event_id").fill_null(0) + 1).rolling_prod(window_size=horizon, min_periods=1).over("event_id").alias(f"cum_ret_factor_temp"))
        announce_day_data = data.filter(pl.col('days_to_announcement') == 0);
        if announce_day_data.is_empty(): print(f"Warning (PEAD Prep): No announcement day data found for horizon {horizon}."); return None, None
        final_data = announce_day_data.with_columns((pl.col(f"cum_ret_factor_temp") - 1).alias(f"target_cum_ret_h{horizon}")).drop_nulls(subset=[f"target_cum_ret_h{horizon}"])
        if final_data.is_empty(): print(f"Warning (PEAD Prep): No valid target data after calculation for horizon {horizon}."); return None, None
        X = final_data.select(feature_cols); y = final_data.get_column(f"target_cum_ret_h{horizon}"); return X, y
    def fit(self, data: pl.DataFrame, feature_cols: List[str]):
        ...
        for horizon in self.time_horizons:
            ...
            try:
                ...
                if X_pl is None or y_pl is None or X_pl.is_empty(): print(f"    No data for horizon {horizon}. Skipping."); continue
                X_np = X_pl.select([pl.col(c).cast(pl.Float64, strict=False) for c in self.feature_names_in_]).to_numpy(); y_np = y_pl.cast(pl.Float64).to_numpy()
                if np.isnan(X_np).any(): print(f"    Imputing NaNs for horizon {horizon}..."); imputer = SimpleImputer(strategy='median'); X_np = imputer.fit_transform(X_np); self.imputers[horizon] = imputer
                if X_np.size > 0: self.models[horizon].fit(X_np, y_np); print(f"    PEAD model {horizon}d fitted ({len(X_np)} samples).")
                else: print(f"    No samples remaining for horizon {horizon}.")
            except Exception as e: print(f"    Error training PEAD {horizon}d: {e}"); traceback.print_exc()
        print("EarningsDriftModel fitting complete."); return self
    def predict(self, data: pl.DataFrame) -> pl.DataFrame:
        ...
        if announce_days.is_empty(): print("Warning: No announcement day data."); pred_cols_schema = {f'pred_drift_h{h}': pl.Float64 for h in self.time_horizons}; announce_schema = announce_days.schema; return pl.DataFrame(schema={**announce_schema, **pred_cols_schema})
        ...
        try: X_np = X_pl.select([pl.col(c).cast(pl.Float64, strict=False) for c in self.feature_names_in_]).to_numpy()
        except Exception as e: raise ValueError(f"Failed to convert PEAD prediction features to NumPy: {e}")
        pred_expressions = []
        for horizon in self.time_horizons:
             ...
             if horizon in self.models:
                 try:
                     ...
                     if np.isnan(X_np_h).any():
                          if horizon in self.imputers: X_np_h = self.imputers[horizon].transform(X_np_h)
                          else: print(f"  Warning: No stored imputer for horizon {horizon}. Imputing PEAD predict with temporary median."); temp_imputer = SimpleImputer(strategy='median'); X_np_h = temp_imputer.fit_transform(X_np_h)
                          if np.isnan(X_np_h).any(): print(f"  Warning: NaNs remain after imputation for horizon {horizon}.")
                     if X_np_h.size > 0: predictions = self.models[horizon].predict(X_np_h); pred_expressions.append(pl.Series(pred_col, predictions))
                     else: pred_expressions.append(pl.lit(None).cast(pl.Float64).alias(pred_col))
                 except Exception as e: print(f"Error predicting PEAD {horizon}d: {e}"); pred_expressions.append(pl.lit(None).cast(pl.Float64).alias(pred_col))
             else: pred_expressions.append(pl.lit(None).cast(pl.Float64).alias(pred_col))
        predictions_df = announce_days.hstack(pred_expressions); return predictions_df
    def evaluate(self, data: pl.DataFrame) -> Dict[int, Dict[str, Any]]:
        print("\n--- Evaluating EarningsDriftModel ---"); results: Dict[int, Dict[str, Any]] = {};
        ...
        for horizon in self.time_horizons:
            ...
            try:
                ...
                if X_test_pl is None or y_test_pl is None or X_test_pl.is_empty(): print("    No test data available."); results[horizon] = {'MSE': np.nan, 'RMSE': np.nan, 'R2': np.nan, 'Direction Accuracy': np.nan, 'N': 0, 'Error': 'No data'}; continue
                X_test_np = X_test_pl.select([pl.col(c).cast(pl.Float64, strict=False) for c in self.feature_names_in_]).to_numpy(); y_test_np = y_test_pl.cast(pl.Float64).to_numpy()
                if np.isnan(X_test_np).any():
                    if horizon in self.imputers: X_test_np = self.imputers[horizon].transform(X_test_np)
                    else: print(f"    Warning: No stored imputer for horizon {horizon}. Imputing with temporary median."); temp_imputer = SimpleImputer(strategy='median'); X_test_np = temp_imputer.fit_transform(X_test_np)
                    if np.isnan(X_test_np).any(): print(f"  Warning: NaNs remain after imputation for horizon {horizon}.")
                if horizon not in self.models: print("    Model not available."); results[horizon] = {'MSE': np.nan, 'RMSE': np.nan, 'R2': np.nan, 'Direction Accuracy': np.nan, 'N': 0, 'Error': 'Model not trained'}; continue
                y_pred_np = np.array([])
                if X_test_np.size > 0: y_pred_np = self.models[horizon].predict(X_test_np)
                else: print("    Test data empty after prep.")
                valid_mask = np.isfinite(y_test_np) & np.isfinite(y_pred_np); y_test_v, y_pred_v = y_test_np[valid_mask], y_pred_np[valid_mask]; mse, r2, dir_acc = np.nan, np.nan, np.nan
                ...
                results[horizon] = {'MSE': mse, 'RMSE': np.sqrt(mse), 'R2': r2, 'Direction Accuracy': dir_acc, 'N': len(y_test_v)}; print(f"    PEAD {horizon}d: MSE={mse:.6f}, R2={r2:.4f}, DirAcc={dir_acc:.4f}, N={len(y_test_v)}")
